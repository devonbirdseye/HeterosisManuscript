...
##GOI list
```{r GOIs list}
GOIs <- list(PE.ribo, PhANGs, PhAPGs, plastid, NE.PtRibo, CytoRibo, EtBio, MAT, ACS, ACO, acs2, acs6, acs7)
names(GOIs) <- c("PE.ribo", "PhANGs", "PhAPGs", "plastid", "NE.PtRibo", "CytoRibo", "EtBio", "MAT", "ACS", "ACO", "acs2", "acs6", "acs7")
saveRDS(GOIs, "GOIs")
GOIs <- readRDS("GOIs")
```
...
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$acs7),"Category"] <- "acs7"
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$acs7),"Order"] <- 5
###Re-order
tmt.SL.acs <- tmt.SL.acs[(order(tmt.SL.acs$Order)),]
###factor
tmt.SL.acs$Category <- factor(tmt.SL.acs$Category, levels = c("Other", "Ethylene biosynthesis", "acs2", "acs6", "acs7"))
```
...
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$acs7),"Category"] <- "acs7"
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$acs7),"Order"] <- 5
###Re-order
cpm.SL.acs <- cpm.SL.acs[(order(cpm.SL.acs$Order)),]
###factor
cpm.SL.acs$Category <- factor(cpm.SL.acs$Category, levels = c("Other", "Ethylene biosynthesis", "acs2", "acs6", "acs7"))
```
