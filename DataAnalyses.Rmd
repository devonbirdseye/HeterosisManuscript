...
nrow(test[test$Gene %in% c(NE.PtRibo, PE.ribo),])
nrow(test)
```
```{r}
test <- table1[table1$Gene%in% c(PhANGs, PhAPGs),]
median(test$PearsonCor)
```
```{r}
test <- table2[table2$Gene%in% c(PhANGs, PhAPGs),]
median(test$PearsonCor)
```
...
cpm.RIL.6H.Hyb2MP.cor <- CorFun(cpm.RIL.6H.Hyb2MP, 2:16, c(PlantHeights.ril.MP, PlantHeights.6h.MP))
#Set GOIs
table2 <- cpm.RIL.6H.Hyb2MP.cor[,c(1, 17)]
colnames(table2) <- c("Gene", "PearsonCor")
table2$Category <- "Other"
table2$Category[table2$Gene %in% GOIs$plastid] <- "Plastid"
table2$Category[table2$Gene %in% GOIs$PE.ribo] <- "PE plastid ribosome"
table2$Category[table2$Gene %in% GOIs$NE.PtRibo] <- "NE plastid ribosome"
table2$Category[table2$Gene %in% GOIs$PhANGs] <- "PhANGs"
table2$Category[table2$Gene %in% GOIs$PhAPGs] <- "PhAPGs"
table2$Category[table2$Gene %in% GOIs$CytoRibo] <- "Cytosolic ribosome"
table2$Category[table2$Gene %in% GOIs$EtBio] <- "Ethylene biosynthesis"
table2$Category[table2$Gene %in% GOIs$acs2] <- "acs2"
table2$Category[table2$Gene %in% GOIs$acs6] <- "acs6"
#Re-order
table2 <- table2[(order(-table2$PearsonCor)),]
```
```{r}
table2
```
