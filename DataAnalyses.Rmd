...
tmt.SL$HP <- NA
for(i in 1:nrow(tmt.SL)){
  if(tmt.SL[i,"B73.Mo17"] > 1.0){
    tmt.SL[i,"HP"] <- tmt.SL[i, "B73avg"]
  }
  else{tmt.SL[i,"HP"] <- tmt.SL[i, "Mo17avg"]}
}
tmt.SL$B73xMo17.HP <- tmt.SL$HYBavg/tmt.SL$HP
tmt.SL$l2.B73xMo17.HP <- log2(tmt.SL$B73xMo17.HP)

tmt.LB$HP <- NA
for(i in 1:nrow(tmt.LB)){
  if(tmt.LB[i,"B73.Mo17"] > 1.0){
    tmt.LB[i,"HP"] <- tmt.LB[i, "B73avg"]
  }
  else{tmt.LB[i,"HP"] <- tmt.LB[i, "Mo17avg"]}
}
tmt.LB$B73xMo17.HP <- tmt.LB$HYBavg/tmt.LB$HP
tmt.LB$l2.B73xMo17.HP <- log2(tmt.LB$B73xMo17.HP)

tmt.6H$HP <- NA
for(i in 1:nrow(tmt.6H)){
  if(tmt.6H[i,"B73.Mo17"] > 1.0){
    tmt.6H[i,"HP"] <- tmt.6H[i, "B73"]
  }
  else{tmt.6H[i,"HP"] <- tmt.6H[i, "Mo17"]}
}
tmt.6H$B73xMo17.HP <- tmt.6H$B73xMo17/tmt.6H$HP
tmt.6H$l2.B73xMo17.HP <- log2(tmt.6H$B73xMo17.HP)

tmt.RIL$HP <- NA
for(i in 1:nrow(tmt.RIL)){
  if(tmt.RIL[i,"BM.B73.Mo17"] > 1.0){
    tmt.RIL[i,"HP"] <- tmt.RIL[i, "BM.B73"]
  }
  else{tmt.RIL[i,"HP"] <- tmt.RIL[i, "BM.Mo17"]}
}
tmt.RIL$B73xMo17.HP <- tmt.RIL$BM.BM/tmt.RIL$HP
tmt.RIL$l2.B73xMo17.HP <- log2(tmt.RIL$B73xMo17.HP)
...
tmt.RIL$plot <- "D"
#trim
tmt.SL.HP <- tmt.SL[,c("plot", "Gene", "l2.B73xMo17.HP")]
tmt.LB.HP <- tmt.LB[,c("plot", "Gene", "l2.B73xMo17.HP")]
tmt.6H.HP <- tmt.6H[,c("plot", "Gene", "l2.B73xMo17.HP")]
tmt.RIL.HP <- tmt.RIL[,c("plot", "Gene", "l2.B73xMo17.HP")]
#bind
tmt.HP <- rbind(tmt.SL.HP,tmt.LB.HP)
tmt.HP <- rbind(tmt.HP,tmt.6H.HP)
tmt.HP <- rbind(tmt.HP,tmt.RIL.HP)
```

```{r plot tmt.HP.plastid.all4}
 plot<-unique(tmt.HP[,"plot"])
  plotList<-vector(mode = "list", length = length(plot))
  plotLab<-unique(tmt.HP[,"plot"])
  for(i in 1:length(plot)){
    df<-tmt.HP[which(tmt.HP[,"plot"]==plot[i]),]
    ###Set limits
df.lims <- df
df.lims$l2.B73xMo17.HP[df.lims$l2.B73xMo17.HP <= -.75] <- -.75
df.lims$l2.B73xMo17.HP[df.lims$l2.B73xMo17.HP >= .75] <- .75
...
df.lims.plastid$Category <- factor(df.lims.plastid$Category, levels = c("Other", "Plastid-localized", "PhANGs", "NE plastid ribosome", "PE plastid ribosome", "PhAPGs"))
    #scatter plot with marginal density curves
    df.lims.plastid.den <- ggplot(df.lims.plastid, mapping=aes(x=l2.B73xMo17.HP, color=Category))+
   geom_density(size=1)+
      xlim(-.75,.75)+
    ...
    xscale <- ggplot(df.lims.plastid, mapping=aes(x=l2.B73xMo17.HP, color=Category))+
   geom_density(size=1)+
      xlim(-.75,.75)+
      coord_cartesian(expand = F)+
...
```{r}
#Subset hyb/MP values from protein data
tmt.RIL.6H.Par2Par <- merge.data.frame(tmt.RIL[,c(6,136:144)], tmt.6H[,c(6,60:65)], by.x = "Accession", by.y = "Accession", all = F)
#Make new gene column to merge with RNA data
tmt.RIL.6H.Par2Par$Gene <- substr(tmt.RIL.6H.Par2Par$Accession, start = 1, stop = 14)

#Subset hyb/MP values from RNA data
cpm.RIL.Par2Par <- cpm.RIL[,c(1,85:93)]
cpm.6H.Par2Par <- cpm.6H[,c(1,48:53)]
cpm.RIL.6H.Par2Par <- merge.data.frame(cpm.RIL[,c(1,76:84)], cpm.6H[,c(1,42:47)], by.x = "Gene", by.y = "Gene", all = F)
#Combine tmt and cpm
tmt.cpm.RIL.6H.Par2Par <- merge.data.frame(tmt.RIL.6H.Par2Par, cpm.RIL.6H.Par2Par, by.x = "Gene", by.y = "Gene", all=F)
#Run correlation analysis between protein hyb/mp and RNA hyb/mp values
tmt.cpm.RIL.6H.Par2Par.cor2 <- CorFun2(tmt.cpm.RIL.6H.Par2Par, 3:17, 18:32)
```
...
```
```{r plot tmt.cpm.RIL.6H.Par2Par.cor2}
tmt.cpm.RIL.6H.Par2Par.cor2$Category <- "Other"
tmt.cpm.RIL.6H.Par2Par.cor2$Order <- 1
tmt.cpm.RIL.6H.Par2Par.cor2[(tmt.cpm.RIL.6H.Par2Par.cor2$Gene %in% GOIs$CytoRibo),"Category"] <- "Cytosolic ribosome"
tmt.cpm.RIL.6H.Par2Par.cor2[(tmt.cpm.RIL.6H.Par2Par.cor2$Gene %in% GOIs$CytoRibo),"Order"] <- 2
tmt.cpm.RIL.6H.Par2Par.cor2[(tmt.cpm.RIL.6H.Par2Par.cor2$Gene %in% GOIs$NE.PtRibo),"Category"] <- "NE plastid ribosome"
tmt.cpm.RIL.6H.Par2Par.cor2[(tmt.cpm.RIL.6H.Par2Par.cor2$Gene %in% GOIs$NE.PtRibo),"Order"] <- 3
tmt.cpm.RIL.6H.Par2Par.cor2[(tmt.cpm.RIL.6H.Par2Par.cor2$Gene %in% GOIs$PE.ribo),"Category"] <- "PE plastid ribosome"
tmt.cpm.RIL.6H.Par2Par.cor2[(tmt.cpm.RIL.6H.Par2Par.cor2$Gene %in% GOIs$PE.ribo),"Order"] <- 4
tmt.cpm.RIL.6H.Par2Par.cor2[(tmt.cpm.RIL.6H.Par2Par.cor2$Gene %in% GOIs$PhANGs),"Category"] <- "PhANGs"
tmt.cpm.RIL.6H.Par2Par.cor2[(tmt.cpm.RIL.6H.Par2Par.cor2$Gene %in% GOIs$PhANGs),"Order"] <- 5
tmt.cpm.RIL.6H.Par2Par.cor2[(tmt.cpm.RIL.6H.Par2Par.cor2$Gene %in% GOIs$PhAPGs),"Category"] <- "PhAPGs"
tmt.cpm.RIL.6H.Par2Par.cor2[(tmt.cpm.RIL.6H.Par2Par.cor2$Gene %in% GOIs$PhAPGs),"Order"] <- 6
tmt.cpm.RIL.6H.Par2Par.cor2 <- tmt.cpm.RIL.6H.Par2Par.cor2[(order(tmt.cpm.RIL.6H.Par2Par.cor2$Order)),]
tmt.cpm.RIL.6H.Par2Par.cor2$Category <- factor(tmt.cpm.RIL.6H.Par2Par.cor2$Category, levels = c("Other", "Cytosolic ribosome", "NE plastid ribosome", "PE plastid ribosome", "PhANGs", "PhAPGs"))
ggplot(tmt.cpm.RIL.6H.Par2Par.cor2, mapping=aes(y=Category, x=cor, fill=Category))+
  geom_density_ridges(alpha=0.8) +
  theme_ridges() +
  theme(legend.position = "none", axis.title.y = element_blank(), axis.title.x = element_text(hjust = 0.5, size=10), axis.text = element_text(size=10))+
  coord_cartesian(expand = F)+
  ...
  xlim(-1,1)+
  scale_fill_manual(values=colors.pc)
ggsave(filename =paste0(PathToPlots, "FigureS9.P2P.pdf"), dpi=300, width=3.25, height=2)
```
...
```{r SupTab2a}
#merge pathview data with MaizeGDB info
...
```{r SDS1 protein}
SDS1a <- Reduce(function(x,y) merge(x,y,by="Accession",all=TRUE) ,list(tmt.SL[,c("Accession", "HYB.MP")], tmt.LB[,c("Accession", "HYB.MP")], tmt.6H[,c("Accession", "B73xMo17.MP")], tmt.RIL[,c("Accession", "BM.BM.MP")]))

colnames(SDS1a) <- c("Accession", "Protein_SeedlingLeaf_BxM.MP", "Protein_LeafBlade_BxM.MP", "Protein_6Hybrids_BxM.MP", "Protein_RILhybrids_BxM.MP")
```
```{r SDS1 rna}
SDS1b <- Reduce(function(x,y) merge(x,y,by="Gene",all=TRUE) ,list(cpm.SL[,c("Gene", "B73xMo17.MP")], cpm.LB[,c("Gene", "B73xMo17.MP")], cpm.6H[,c("Gene", "B73xMo17.MP")], cpm.RIL[,c("Gene", "B73xMo17.MP")]))

colnames(SDS1b) <- c("Gene", "RNA_SeedlingLeaf_BxM.MP", "RNA_LeafBlade_BxM.MP", "RNA_6Hybrids_BxM.MP", "RNA_RILhybrids_BxM.MP")
```
```{r SDS1 combine protein and rna}
SDS1a$Gene <- substr(SDS1a$Accession, start = 1, stop = 14)
SDS1 <- merge.data.frame(SDS1a, SDS1b, by.x = "Gene", by.y = "Gene", all = T)
SDS1 <- SDS1[order(-SDS1$Protein_SeedlingLeaf_BxM.MP),]
```
```{r SDS1 add category column}
SDS1$Category <- "Other"
SDS1$Category[SDS1$Gene %in% GOIs$plastid] <- "Plastid"
SDS1$Category[SDS1$Gene %in% GOIs$PE.ribo] <- "PE plastid ribosome"
SDS1$Category[SDS1$Gene %in% GOIs$NE.PtRibo] <- "NE plastid ribosome"
SDS1$Category[SDS1$Gene %in% GOIs$PhANGs] <- "PhANGs"
SDS1$Category[SDS1$Gene %in% GOIs$PhAPGs] <- "PhAPGs"
SDS1$Category[SDS1$Gene %in% GOIs$CytoRibo] <- "Cytosolic ribosome"
SDS1$Category[SDS1$Gene %in% GOIs$SAMS] <- "Ethylene biosynthesis - SAM synthase"
SDS1$Category[SDS1$Gene %in% GOIs$ACS] <- "Ethylene biosynthesis - ACC synthase"
SDS1$Category[SDS1$Gene %in% GOIs$ACO] <- "Ethylene biosynthesis - ACC oxidase"
SDS1$Category[SDS1$Gene %in% GOIs$acs2] <- "Ethylene biosynthesis - ACS2"
SDS1$Category[SDS1$Gene %in% GOIs$acs6] <- "Ethylene biosynthesis - ACS6"
SDS1$Category[SDS1$Gene %in% GOIs$AtACO1hom] <- "Ethylene biosynthesis - AtACO1 homolog"
SDS1$Category[SDS1$Gene %in% GOIs$AtACO2hom] <- "Ethylene biosynthesis - AtACO2 homolog"
SDS1$Category[SDS1$Gene %in% GOIs$AtSAM1AtSAM2hom] <- "Ethylene biosynthesis - AtSAM1/2 homolog"

write.csv(SDS1[,c(1,2,11,3:10)], "SplementalDataSet1.csv", row.names = F)
```
