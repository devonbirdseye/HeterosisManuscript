...
tmt.RIL$M0014.B73 <- rowMeans(tmt.RIL[,20:23], na.rm = T)
tmt.RIL$M0014.Mo17<- rowMeans(tmt.RIL[,24:27], na.rm = T)
tmt.RIL$M0014.14 <- rowMeans(tmt.RIL[,28:31], na.rm = T)
tmt.RIL$M0014.14B <- rowMeans(tmt.RIL[,32:35], na.rm = T)
tmt.RIL$M0014.14M <- rowMeans(tmt.RIL[,36:39], na.rm = T)
tmt.RIL$M0016.B73 <- rowMeans(tmt.RIL[,40:43], na.rm = T)
tmt.RIL$M0016.Mo17<- rowMeans(tmt.RIL[,44:47], na.rm = T)
tmt.RIL$M0016.16 <- rowMeans(tmt.RIL[,48:51], na.rm = T)
tmt.RIL$M0016.16B <- rowMeans(tmt.RIL[,52:55], na.rm = T)
tmt.RIL$M0016.16M <- rowMeans(tmt.RIL[,56:59], na.rm = T)
tmt.RIL$M0021.B73 <- rowMeans(tmt.RIL[,60:63], na.rm = T)
tmt.RIL$M0021.Mo17<- rowMeans(tmt.RIL[,64:67], na.rm = T)
tmt.RIL$M0021.21 <- rowMeans(tmt.RIL[,68:71], na.rm = T)
tmt.RIL$M0021.21B <- rowMeans(tmt.RIL[,72:75], na.rm = T)
tmt.RIL$M0021.21M <- rowMeans(tmt.RIL[,76:79], na.rm = T)
tmt.RIL$M0317.B73 <- rowMeans(tmt.RIL[,80:83], na.rm = T)
tmt.RIL$M0317.Mo17<- rowMeans(tmt.RIL[,84:87], na.rm = T)
tmt.RIL$M0317.317 <- rowMeans(tmt.RIL[,88:91], na.rm = T)
tmt.RIL$M0317.317B <- rowMeans(tmt.RIL[,92:95], na.rm = T)
tmt.RIL$M0317.317M <- rowMeans(tmt.RIL[,96:99], na.rm = T)
#Calculate hyb/mp ratios
...
PlantHeights.6h.MP <- as.numeric(PlantHeights.6H[,11:16])
...
PlantHeights.6h.BxM <- as.numeric(PlantHeights.6H[,17:22])
...
PlantHeights.ril.MP <- as.numeric(PlantHeights.RIL[,16:24])
...
PlantHeights.ril.BxM <- as.numeric(PlantHeights.RIL[,25:33])
```
...
cpm.BxM.SL.OG <- cpm.SL
cpm.BxM.SL.6h <- cpm.6H
cpm.BxM.SL.ril <- cpm.RIL
...
cpm.6H.bm <- cpm.6H[,c("Gene", "l2.B73xMo17.MP", "l2.B73.Mo17")]
cpm.6H.mb <- cpm.6H[,c("Gene", "l2.Mo17xB73.MP", "l2.Mo17.B73")]
cpm.6H.a682xb <- cpm.6H[,c("Gene", "l2.A682xB73.MP", "l2.A682.B73")]
cpm.6H.a682xm <- cpm.6H[,c("Gene", "l2.A682xMo17.MP", "l2.A682.Mo17")]
cpm.6H.b84xb <- cpm.6H[,c("Gene", "l2.B84xB73.MP", "l2.B84.B73")]
cpm.6H.b84xm <- cpm.6H[,c("Gene", "l2.B84xB73.MP", "l2.B84.B73")]
...
```
#Figure cpm.RIL.6H.Hyb2MP.cor
```{r setup dataframe cpm.RIL.6H.Hyb2MP.cor}
...
###Add density curves
    ...
    densityplotY<-axis_canvas(tmt.cpm.RIL.6H.Hyb2MP.cor.plot, axis="y", coord_flip = T)+
      ...
      coord_flip(expand = F)
    #put two plots together
      q <- insert_xaxis_grob(tmt.cpm.RIL.6H.Hyb2MP.cor.plot, densityplotX, grid::unit(0.275, "null"), position = "top")
      q <- insert_yaxis_grob(q, densityplotY, grid::unit(0.275, "null"), position = "right")
ggsave(plot=q, filename =paste0(PathToPlots, "tmt.cpm.RIL.6H.Hyb2MP.cor.pdf"),
...
###Add density curves
    ...
    densityplotY<-axis_canvas(tmt.SL.lims.etbio.scat, axis="y", coord_flip = T)+
      ...
      coord_flip()
    #put two plots together
      q <- insert_xaxis_grob(tmt.SL.lims.etbio.scat, densityplotX, grid::unit(0.275, "null"), position = "top")
      q <- insert_yaxis_grob(q, densityplotY, grid::unit(0.275, "null"), position = "right")
    #save
     # ggsave(plot=q, filename = "tmt.SL.lims.etbio.scat.png")
  ggsave(plot=q, filename = paste0(PathToPlots, "tmt.SL.EtBio.pdf"),  dpi=300, width=4.75, height = 5, scale = 1, units = "in")
```
...
pathview(gene.data = tmt.BxM.6H.entrez.nodups.trim, pathway.id = "zma00592", species = "zma",  bins = 20, limit = 0.5, out.suffix="tmt.BxM.zma00592")
```
...
#RNA correlations
cpm.6H.cor <- CorFun(cpm.6H, 54:59, PlantHeights.6h.MP)
cpm.RIL.cor <- CorFun(cpm.RIL, 94:102, PlantHeights.ril.MP)
#Combine RNA correlations
cpm.cor <- merge.data.frame(cpm.6H.cor[,c("Gene", "cor")], cpm.RIL.cor[,c("Gene", "cor")], by.x="Gene", by.y="Gene", all=T)
colnames(cpm.cor) <- c("Gene", "Cor.6h", "Cor.RIL")
#Define GOIs
cpm.cor <- merge.data.frame(cpm.cor, mgdb[,c(1,12)], by.x = "Gene", by.y = "v4_gene_model", all.x = T, all.y = F)
cpm.cor$GOI <- "Other"
cpm.cor$Order <- 1
...
p <- ggplot(cpm.cor, aes(x = Cor.6h, y = Cor.RIL, color=GOI))+
  geom_point(size=3)+
  theme_bw()+
  ...
  ggtitle("6 hybrids vs RIL hybrids RNA correlations")
###Add density curves
    densityplotX<-axis_canvas(p, axis="x")+
      geom_density(aes(cpm.cor[cpm.cor$Order<7,"Cor.6h"], color=cpm.cor[cpm.cor$Order<7,"GOI"]), size=0.75)+
    ...
    densityplotY<-axis_canvas(p, axis="y", coord_flip = T)+
      geom_density(aes(cpm.cor[cpm.cor$Order<7,"Cor.RIL"], color=cpm.cor[cpm.cor$Order<7,"GOI"]), size=0.75)+
      scale_color_manual(values=c("Other"="black","PhANGs"="green", "PhAPGs"="blue", "NE.PtRibo"= "pink", "PE.ribo"= "red", "CytoRibo"= "purple", "MAT"= "yellow", "ACS"= "orange", "ACO"="brown"), name="GOI")+
      scale_linetype_manual(values=c("dotted", "solid"))+
      geom_vline(xintercept=0, linetype="dashed", color = "gray60", size=0.5)+
      theme_void()+
    ...
    #put two plots together
      ...
      q <- insert_yaxis_grob(q, densityplotY, grid::unit(0.275, "null"), position = "right")
ggsave(plot=q, "cpm.6H.RIL.cor.png")
```
...
tmt.cor$Order[tmt.cor$Gene%in%GOIs$PhANGs] <- 2
tmt.cor$GOI[tmt.cor$Gene%in%GOIs$PhAPGs] <- "PhAPGs"
tmt.cor$Order[tmt.cor$Gene%in%GOIs$PhAPGs] <- 3
tmt.cor$GOI[tmt.cor$Gene%in%GOIs$NE.PtRibo] <- "NE.PtRibo"
tmt.cor$Order[tmt.cor$Gene%in%GOIs$NE.PtRibo] <- 4
tmt.cor$GOI[tmt.cor$Gene%in%GOIs$PE.ribo] <- "PE.ribo"
tmt.cor$Order[tmt.cor$Gene%in%GOIs$PE.ribo] <- 5
tmt.cor$GOI[tmt.cor$Gene%in%GOIs$CytoRibo] <- "CytoRibo"
tmt.cor$Order[tmt.cor$Gene%in%GOIs$CytoRibo] <- 6
tmt.cor$GOI[tmt.cor$Gene%in%GOIs$MAT] <- "MAT"
tmt.cor$Order[tmt.cor$Gene%in%GOIs$MAT] <- 7
tmt.cor$GOI[tmt.cor$Gene%in%GOIs$ACS] <- "ACS"
tmt.cor$Order[tmt.cor$Gene%in%GOIs$ACS] <- 8
tmt.cor$GOI[tmt.cor$Gene%in%GOIs$ACO] <- "ACO"
tmt.cor$Order[tmt.cor$Gene%in%GOIs$ACO] <- 9
tmt.cor <- tmt.cor[order(tmt.cor$Order),]
tmt.cor$GOI <- factor(tmt.cor$GOI, levels = c("Other", "PhANGs", "PhAPGs", "NE.PtRibo", "PE.ribo", "CytoRibo", "MAT", "ACS", "ACO"))
```
...
###Add density curves
    densityplotX<-axis_canvas(p, axis="x")+
      geom_density(aes(tmt.cor[tmt.cor$Order<7,"Cor.6h"], color=tmt.cor[tmt.cor$Order<7,"GOI"]), size=0.75)+
    scale_color_manual(values=c("Other"="black","PhANGs"="green", "PhAPGs"="blue", "NE.PtRibo"= "pink", "PE.ribo"= "red", "CytoRibo"= "purple", "MAT"= "yellow", "ACS"= "orange", "ACO"="brown"), name="GOI")+
      geom_vline(xintercept=0, linetype="dashed", color = "gray60", size=0.5)+
      theme_void()+
      theme(legend.title = element_blank())
    #set up marginal density plot Y axis
    densityplotY<-axis_canvas(p, axis="y", coord_flip = T)+
      geom_density(aes(tmt.cor[tmt.cor$Order<7,"Cor.RIL"], color=tmt.cor[tmt.cor$Order<7,"GOI"]), size=0.75)+
      scale_color_manual(values=c("Other"="black","PhANGs"="green", "PhAPGs"="blue", "NE.PtRibo"= "pink", "PE.ribo"= "red", "CytoRibo"= "purple", "MAT"= "yellow", "ACS"= "orange", "ACO"="brown"), name="GOI")+
      scale_linetype_manual(values=c("dotted", "solid"))+
      geom_vline(xintercept=0, linetype="dashed", color = "gray60", size=0.5)+
      theme_void()+
    ...
    #put two plots together
      q <- insert_xaxis_grob(p, densityplotX, grid::unit(0.275, "null"), position = "top")
      q <- insert_yaxis_grob(q, densityplotY, grid::unit(0.275, "null"), position = "right")
ggsave(plot=q, "tmt.6H.RIL.cor.png")
```
#GOI correlations
```{r GOIcor protein}
#make dataframe
...
```{r setup dataframe tmt.cpm.BxM.LB.plastid}
###Set limits
tmt.LB.lims <- tmt.LB
...
FigLB.SL.plotlist <- list(A, B, legend)
FigLB.SL.design <- "AB
                 CC"
FigLB.SL.plotpanel<-patchwork::wrap_plots(A=FigLB.SL.plotlist[[1]], B=FigLB.SL.plotlist[[2]], C=FigLB.SL.plotlist[[3]],  nrow = 2, heights = c(3,1.2), design = FigLB.SL.design)
ggsave(plot=FigLB.SL.plotpanel, filename =paste0(PathToPlots, "LeafBladeVsSeedlingLeaf.Hyb2MP.pdf"), dpi=300, width=4.75, height=3)
```
```{r plot LeafBladeVsSeedlingLeaf.Parent2Parent}
###Set limits
tmt.LB.SL.lims <- tmt.LB.SL
...
FigLB.SL.design <- "AB
                 CC"
FigLB.SL.plotpanel<-patchwork::wrap_plots(A=FigLB.SL.plotlist[[1]], B=FigLB.SL.plotlist[[2]], C=FigLB.SL.plotlist[[3]],  nrow = 2, heights = c(3,1.2), design = FigLB.SL.design)
ggsave(plot=FigLB.SL.plotpanel, filename =paste0(PathToPlots, "LeafBladeVsSeedlingLeaf.Parent2Parent.pdf"), dpi=300, width=4.75, height=3)
```
