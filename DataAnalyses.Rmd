...
##GOI list
```{r GOIs list}
GOIs <- list(PE.ribo, PhANGs, PhAPGs, plastid, NE.PtRibo, CytoRibo, EtBio, acs2, acs6)
names(GOIs) <- c("PE.ribo", "PhANGs", "PhAPGs", "plastid", "NE.PtRibo", "CytoRibo", "EtBio", "acs2", "acs6")
saveRDS(GOIs, "GOIs")
GOIs <- readRDS("GOIs")
```
...
colors.ea <- c("Other"="grey", "Ethylene biosynthesis"="black", "acs2"="#24ff24", "acs6"="#ff6db6", "acs7"="#b66dff")
...
tmt.SL.acs$Order <- 1
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$EtBio),"Category"] <- "Ethylene biosynthesis"
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$EtBio),"Order"] <- 2
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$acs2),"Category"] <- "acs2"
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$acs2),"Order"] <- 3
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$acs6),"Category"] <- "acs6"
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$acs6),"Order"] <- 4
###Re-order
tmt.SL.acs <- tmt.SL.acs[(order(tmt.SL.acs$Order)),]
###factor
tmt.SL.acs$Category <- factor(tmt.SL.acs$Category, levels = c("Other", "Ethylene biosynthesis", "acs2", "acs6"))
```
...
cpm.SL.acs$Order <- 1
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$EtBio),"Category"] <- "Ethylene biosynthesis"
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$EtBio),"Order"] <- 2
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$acs2),"Category"] <- "acs2"
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$acs2),"Order"] <- 3
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$acs6),"Category"] <- "acs6"
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$acs6),"Order"] <- 4
###Re-order
cpm.SL.acs <- cpm.SL.acs[(order(cpm.SL.acs$Order)),]
###factor
cpm.SL.acs$Category <- factor(cpm.SL.acs$Category, levels = c("Other", "Ethylene biosynthesis", "acs2", "acs6"))
```
