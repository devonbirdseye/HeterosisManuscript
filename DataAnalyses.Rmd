...
#save
write.csv(cpm.6H, "cpm.6H.csv", row.names = F)
#Remove genes not detected in one or mor rep
cpm.6H[cpm.6H==0] <- NA
...
#reformat df
cpm.RIL <- dcast(cpm.RIL.tm[,c("gid", "SampleID", "CPM")], gid ~ SampleID)
#Rename columns
colnames(cpm.RIL) <- c("Gene", paste(cpm.RIL.th$Genotype, cpm.RIL.th$Replicate, sep = "."))
#save
write.csv(cpm.RIL, "cpm.RIL.csv", row.names = F)
#Remove genes not detected in one or mor rep
...
#Plant Height data
```{r PlantHeights}
PlantHeights <- read.csv(paste0(PathToData, "SupplementalTable3.csv"), stringsAsFactors = F)
...
#Plastid encoded ribosomal proteins (NCBI Annotation)
...
KEGG.zma00195 <- read.csv(paste0(PathToData, "KEGG.zma00195.csv"), stringsAsFactors = F)
#Get v4 accessions
KEGG.zma00195.digenomic <- join(KEGG.zma00195.digenomic, Entrez)
```
###KEGG - zma00196
```{r KEGG.zma00196}
#Retrieved from KEGG on 02/26/2020
KEGG.zma00196 <- read.csv(paste0(PathToData, "KEGG.zma00196.csv"), stringsAsFactors = F)
#Get v4 accessions
...
plastid <- unique(na.omit(c(friso.v4, PE, PhANGs)))
```
...
```{r}
walley <- read.csv("WalleyS10.RNA.csv", stringsAsFactors = F)
colnames(walley) <- as.character(walley[1,])
walley <- walley[-1,]
walley <- merge.data.frame(walley, mgdb[,c("v4_gene_model", "v3_gene_model")], by.x = "Regulator (TF)", by.y = "v3_gene_model", all=F)
colnames(walley)[5] <- "Reg"
walley <- merge.data.frame(walley, mgdb[,c("v4_gene_model", "v3_gene_model")], by.x = "Target Transcript", by.y = "v3_gene_model", all=F)
colnames(walley)[6] <- "Targ"
walley <- walley[walley$`High Confidence Edge?`=="T",]
```
```{r}
walley.freq <- as.data.frame(table(walley[,c("Reg")]))
colnames(walley.freq) <- c("Reg", "NumberOfTargets")
walley <- merge.data.frame(walley, walley.freq, by.x = "Reg", by.y = "Reg")
```

```{r}
walley.etbio <- distinct(walley[walley$Targ %in% GOIs$EtBio,])
walley.etbio.freq <-  as.data.frame(table(walley.etbio[,c("Reg")]))
colnames(walley.etbio.freq) <- c("Reg", "NumberOfEtBioTargets")
walley.etbio <- merge.data.frame(walley.etbio, walley.etbio.freq, by.x = "Reg", by.y = "Reg")
walley.etbio <- walley.etbio[order(walley.etbio$NumberOfTargets),]
walley.etbio.detected <- merge.data.frame(walley.etbio, cpm.SL.etbio[,c("Gene", "B73xMo17.MP", "ttest")], by.x = "Reg", by.y = "Gene", all=F)
colnames(walley.etbio.detected)[9:10] <- c("Reg_Hyb.MP", "Reg_ttest")
walley.etbio.detected <- merge.data.frame(walley.etbio.detected, cpm.SL.etbio[,c("Gene", "B73xMo17.MP", "ttest", "Category")], by.x = "Targ", by.y = "Gene", all=F)
colnames(walley.etbio.detected)[11:13] <- c("Targ_Hyb.MP", "Targ_ttest", "TargEnzyme")
walley.etbio.detected <- walley.etbio.detected[order(walley.etbio.detected$NumberOfTargets),]
walley.etbio.detected <- merge.data.frame(walley.etbio.detected, table2[,c("Gene", "PearsonCor")], by.x = "Reg", by.y = "Gene", all.x=T, all.y = F)
write.csv(walley.etbio.detected, "walley.DetectedEtBioTFs.csv", row.names = F)
```

```{r}
walley.detected <- merge.data.frame(walley, cpm.SL.etbio[,c("Gene", "B73xMo17.MP", "ttest")], by.x = "Reg", by.y = "Gene", all=F)
colnames(walley.detected)[8:9] <- c("Reg_Hyb.MP", "Reg_ttest")
walley.detected <- merge.data.frame(walley.detected, cpm.SL.etbio[,c("Gene", "B73xMo17.MP", "ttest", "Category")], by.x = "Targ", by.y = "Gene", all=F)
colnames(walley.detected)[10:12] <- c("Targ_Hyb.MP", "Targ_ttest", "TargEnzyme")
walley.detected <- merge.data.frame(walley.detected, table2[,c("Gene", "PearsonCor")], by.x = "Reg", by.y = "Gene", all.x=T, all.y = F)
```

```{r ABA}
ABA <- tibble::tribble(
       ~Gene,                                                                            ~Name,
  100273156L,                               "uncharacterized protein LOC100273156 [KO:K15746]",
  100281136L,                               "uncharacterized protein LOC100281136 [KO:K09840]",
  100281391L,                               "uncharacterized protein LOC100281391 [KO:K09840]",
  100281693L,                                          "beta-carotene hydroxylase [KO:K15746]",
  100282128L,               "sca1; uncharacterized protein LOC100282128 isoform 2 [KO:K09841]",
  100285076L,                                               "zeaxanthin epoxidase [KO:K09838]",
  100384384L,                            "uncharacterized LOC100384384 isoform X2 [KO:K09842]",
  100501444L,            "9-cis-epoxycarotenoid dioxygenase 1, chloroplastic-like [KO:K09840]",
  100501454L,                               "uncharacterized protein LOC100501454 [KO:K09840]",
  103626043L,            "9-cis-epoxycarotenoid dioxygenase 1, chloroplastic-like [KO:K09840]",
  103627610L,             "9-cis-epoxycarotenoid dioxygenase NCED1, chloroplastic [KO:K09840]",
  103627895L,                          "protein LUTEIN DEFICIENT 5, chloroplastic [KO:K15747]",
  103631225L,                         "beta-carotene 3-hydroxylase, chloroplastic [KO:K15746]",
  103631977L,                      "indole-3-acetaldehyde oxidase-like isoform X3 [KO:K09842]",
  103634160L,        "9-cis-epoxycarotenoid dioxygenase NCED4, chloroplastic-like [KO:K09840]",
  103634451L,             "9-cis-epoxycarotenoid dioxygenase NCED4, chloroplastic [KO:K09840]",
  103641704L,                     "zeaxanthin epoxidase, chloroplastic isoform X1 [KO:K09838]",
  103648268L,             "9-cis-epoxycarotenoid dioxygenase NCED4, chloroplastic [KO:K09840]",
  103652265L,             "9-cis-epoxycarotenoid dioxygenase NCED4, chloroplastic [KO:K09840]",
     732775L,                                    "HYD1; beta-carotene hydroxylase [KO:K15746]",
     732819L, "vp14; 9-cis-epoxycarotenoid dioxygenase 1, chloroplastic precursor [KO:K09840]",
     732825L,                                    "HYD3; beta-carotene hydroxylase [KO:K15746]"
  )

ABA <- merge.data.frame(ABA, mgdb[,c("v4_gene_model", "Entrez")], by.x = "Gene", by.y="Entrez", all=F)
ABA <- ABA$v4_gene_model
```
```{r}
GA <- read.delim("GAGenesCornCyc.txt", stringsAsFactors = F)
GA <- GA$Gene.Accession
```
...
pal8 <- "Zm00001d017276"
pal9 <- "Zm00001d017275"
pal10 <- "Zm00001d029015"
```


```{r setup dataframe tmt.SL.zea.vol}
###Set GOIs
tmt.SL.zea <- tmt.SL
tmt.SL.zea$l2.B73xMo17.MP[tmt.SL.zea$l2.B73xMo17.MP <= -1] <- -1
tmt.SL.zea$l2.B73xMo17.MP[tmt.SL.zea$l2.B73xMo17.MP >= 1] <- 1
tmt.SL.zea$MP1 <- rowMeans(tmt.SL.zea[,c("SeedlingLeaf.B73.r1","SeedlingLeaf.Mo17.r1")])
tmt.SL.zea$MP2 <- rowMeans(tmt.SL.zea[,c("SeedlingLeaf.B73.r2","SeedlingLeaf.Mo17.r2")])
tmt.SL.zea$MP3 <- rowMeans(tmt.SL.zea[,c("SeedlingLeaf.B73.r3","SeedlingLeaf.Mo17.r3")])
tmt.SL.zea$ttest <- ttestFun(tmt.SL.zea, c("SeedlingLeaf.BM.r1", "SeedlingLeaf.BM.r2", "SeedlingLeaf.BM.r3"), c("MP1", "MP2", "MP3"))
tmt.SL.zea$l10.ttest <- -(log10(tmt.SL.zea$ttest))
tmt.SL.zea$Category <- "Other"
tmt.SL.zea$Order <- 1
tmt.SL.zea[(tmt.SL.zea$Gene %in% TransZeatin),"Category"] <- "Trans-Zeatin Biosynthesis"
tmt.SL.zea[(tmt.SL.zea$Gene %in% TransZeatin),"Order"] <- 2

###Re-order
tmt.SL.zea <- tmt.SL.zea[(order(tmt.SL.zea$Order)),]
###factor
tmt.SL.zea$Category <- factor(tmt.SL.zea$Category, levels = c("Other", "Trans-Zeatin Biosynthesis"))
```
```{r setup dataframe cpm.SL.zea.vol}
###Set GOIs
cpm.SL.zea <- cpm.SL
cpm.SL.zea$l2.B73xMo17.MP[cpm.SL.zea$l2.B73xMo17.MP <= -2.5] <- -2.5
cpm.SL.zea$l2.B73xMo17.MP[cpm.SL.zea$l2.B73xMo17.MP >= 2.5] <- 2.5
cpm.SL.zea$MP1 <- rowMeans(cpm.SL.zea[,c("B73.1","Mo17.1")])
cpm.SL.zea$MP2 <- rowMeans(cpm.SL.zea[,c("B73.2","Mo17.2")])
cpm.SL.zea$MP3 <- rowMeans(cpm.SL.zea[,c("B73.3","Mo17.3")])
cpm.SL.zea$ttest <- ttestFun(cpm.SL.zea, c("B73xMo17.1", "B73xMo17.2", "B73xMo17.3"), c("MP1", "MP2", "MP3"))
cpm.SL.zea$l10.ttest <- -(log10(cpm.SL.zea$ttest))
cpm.SL.zea$Category <- "Other"
cpm.SL.zea$Order <- 1
cpm.SL.zea[(cpm.SL.zea$Gene %in% TransZeatin),"Category"] <- "Trans-Zeatin Biosynthesis"
cpm.SL.zea[(cpm.SL.zea$Gene %in% TransZeatin),"Order"] <- 2
###Re-order
cpm.SL.zea <- cpm.SL.zea[(order(cpm.SL.zea$Order)),]
###factor
cpm.SL.zea$Category <- factor(cpm.SL.zea$Category, levels = c("Other", "Trans-Zeatin Biosynthesis"))
```
```{r plot tmt.cpm.SL.zea.vol}
#Scatterplot - tmt
tmt.SL.zea.vol <- ggplot(tmt.SL.zea, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
...
#volterplot - cpm
cpm.SL.zea.vol <- ggplot(cpm.SL.zea, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=c("black", "red"), name="Category")+
  scale_shape_manual(values=c(1,19))+
    theme_classic()+
    ...
    theme(aspect.ratio=1)+
    coord_fixed(expand = F)
cpm.SL.zea.vol.legend <- get_legend(cpm.SL.zea.vol)
cpm.SL.zea.vol <- cpm.SL.zea.vol+theme(legend.position = "none")

#Wrap into panel
design <- "AB
           CC"
plotpanel<-patchwork::wrap_plots(A=tmt.SL.zea.vol, B=cpm.SL.zea.vol, C=cpm.SL.zea.vol.legend,  nrow = 2, heights = c(3,1.2), design = design)
ggsave(plot=plotpanel, filename =paste0(PathToPlots, "tmt.cpm.SL.zea.vol.pdf"), dpi=300, width=4.75, height=3)
```
```{r setup dataframe tmt.acs.zea.vol}
###Set GOIs
tmt.acs.zea <- tmt.acs
tmt.acs.zea$l2.ACS.B73[tmt.acs.zea$l2.ACS.B73 <= -1] <- -1
tmt.acs.zea$l2.ACS.B73[tmt.acs.zea$l2.ACS.B73 >= 1] <- 1
tmt.acs.zea$ttest <- ttestFun(tmt.acs.zea, c("B73.acs.B73.r1", "B73.acs.B73.r2", "B73.acs.B73.r3", "B73.acs.B73.r4", "B73.acs.B73.r5"), c("B73.acs.acs.r1", "B73.acs.acs.r2", "B73.acs.acs.r3", "B73.acs.acs.r4", "B73.acs.acs.r5"))
tmt.acs.zea$l10.ttest <- -(log10(tmt.acs.zea$ttest))
tmt.acs.zea$Category <- "Other"
tmt.acs.zea$Order <- 1
tmt.acs.zea[(tmt.acs.zea$Gene %in% TransZeatin),"Category"] <- "Trans-Zeatin Biosynthesis"
tmt.acs.zea[(tmt.acs.zea$Gene %in% TransZeatin),"Order"] <- 2
###Re-order
tmt.acs.zea <- tmt.acs.zea[(order(tmt.acs.zea$Order)),]
###factor
tmt.acs.zea$Category <- factor(tmt.acs.zea$Category, levels = c("Other", "Trans-Zeatin Biosynthesis"))
```
```{r}
tmt.acs.zea.vol <- ggplot(tmt.acs.zea, mapping=aes(x=l2.ACS.B73, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=c("black", "red"), name="Category")+
  scale_shape_manual(values=c(1,19))+
    theme_classic()+
    ...
    theme(aspect.ratio=1)+
    coord_fixed(expand = F)
ggsave(paste0(PathToPlots, "tmt.acs.zea.vol.pdf"), width = 3, height = 3)
```
...
df.lims.plastid$Category <- factor(df.lims.plastid$Category, levels = c("Other", "Plastid-localized", "PhANGs", "NE plastid ribosome", "PE plastid ribosome", "PhAPGs"))
  ...
  scale_color_manual(values=colors.p, name="Category")+
    ...
    theme_classic()+
      ...
      theme(axis.text=element_text(size=10), legend.text=element_text(size=10),
        legend.position="bottom", legend.title = element_blank(), plot.title = element_text(vjust = -3.5, hjust = -0.485), legend.spacing.x = unit(0.1, 'cm'), legend.spacing.y = unit(-3, 'cm'))+
      guides(col=guide_legend(nrow = 2))
...
```
#Figure tmt.RIL.6H.Hyb2MP.cor.densities
```{r setup dataframe tmt.RIL.6H.Hyb2MP.cor.densities}
...
```
```{r plot tmt.RIL.6H.Hyb2MP.cor.densities}
...
tmt.RIL.6H.Hyb2MP.cor.plot <- ggplot(tmt.RIL.6H.Hyb2MP.cor, mapping=aes(y=Category, x=cor, fill=Category))+
  ...
  theme_ridges() +
  theme(legend.position = "none", axis.title.y = element_blank(), axis.title.x = element_text(hjust = 0.5, size=12))+
  coord_cartesian(expand = F)+
...
```
#Figure cpm.RIL.6H.Hyb2MP.cor.densities
```{r setup dataframe cpm.RIL.6H.Hyb2MP.cor.densities}
...
cpm.RIL.6H.Hyb2MP.cor <- CorFun(cpm.RIL.6H.Hyb2MP, 2:16, c(PlantHeights.ril.MP, PlantHeights.6h.MP))
```
```{r plot cpm.RIL.6H.Hyb2MP.cor.densities}
...
cpm.RIL.6H.Hyb2MP.cor[(cpm.RIL.6H.Hyb2MP.cor$Gene %in% GOIs$PhANGs),"Order"] <- 4
cpm.RIL.6H.Hyb2MP.cor <- cpm.RIL.6H.Hyb2MP.cor[(order(cpm.RIL.6H.Hyb2MP.cor$Order)),]
cpm.RIL.6H.Hyb2MP.cor$Category <- factor(cpm.RIL.6H.Hyb2MP.cor$Category, levels = c("Other", "Cytosolic ribosome", "NE plastid ribosome", "PhANGs"))
...
```
#Figure tmt.cpm.RIL.6H.Hyb2MP.cor.densities

```{r tmt.cpm.RIL.6H.Hyb2MP.cor.densities}
tmt.RIL.6H.Hyb2MP.cor$data <- "Protein"
cpm.RIL.6H.Hyb2MP.cor$data <- "RNA"
tmt.cpm.RIL.6H.Hyb2MP.cor <- rbind(tmt.RIL.6H.Hyb2MP.cor[,c(17,18,21)], cpm.RIL.6H.Hyb2MP.cor[,c(1,17,20)])
tmt.cpm.RIL.6H.Hyb2MP.cor$Category <- "Other"
tmt.cpm.RIL.6H.Hyb2MP.cor$Order <- 1
tmt.cpm.RIL.6H.Hyb2MP.cor[(tmt.cpm.RIL.6H.Hyb2MP.cor$Gene %in% GOIs$CytoRibo),"Category"] <- "Cytosolic ribosome"
tmt.cpm.RIL.6H.Hyb2MP.cor[(tmt.cpm.RIL.6H.Hyb2MP.cor$Gene %in% GOIs$CytoRibo),"Order"] <- 2
...
tmt.cpm.RIL.6H.Hyb2MP.cor.plot <- ggplot(tmt.cpm.RIL.6H.Hyb2MP.cor, mapping=aes(y=Category, x=cor, fill=Category,  linetype=data))+
  ...
  theme_ridges() +
  theme(legend.position = "bottom", axis.title.y = element_blank(), axis.title.x = element_text(hjust = 0.5, size=12), legend.justification = c(0.5,1))+
  coord_cartesian(expand = F)+
  xlab("Pearson correlation")+
  xlim(-1,1)
tmt.cpm.RIL.6H.Hyb2MP.cor.plot
ggsave(filename =paste0(PathToPlots, "tmt.cpm.RIL.6H.Hyb2MP.cor.densities.pdf"), dpi=300, width=4.75, height=4)
```
...
tmt.SL.etbio$ttest <- ttestFun(tmt.SL.etbio, c("SeedlingLeaf.BM.r1", "SeedlingLeaf.BM.r2", "SeedlingLeaf.BM.r3"), c("MP1", "MP2", "MP3"))
...
cpm.SL.etbio$ttest <- ttestFun(cpm.SL.etbio, c("B73xMo17.1", "B73xMo17.2", "B73xMo17.3"), c("MP1", "MP2", "MP3"))
...
```
##6H
```{r setup dataframe tmt.6H.EtBio}
###Set GOIs
tmt.6H.etbio <- tmt.6H
tmt.6H.etbio$l2.B73xMo17.MP[tmt.6H.etbio$l2.B73xMo17.MP <= -1] <- -1
tmt.6H.etbio$l2.B73xMo17.MP[tmt.6H.etbio$l2.B73xMo17.MP >= 1] <- 1
tmt.6H.etbio$MP1 <- rowMeans(tmt.6H.etbio[,c("X6Hybrid.B73.r1","X6Hybrid.Mo17.r1")])
tmt.6H.etbio$MP2 <- rowMeans(tmt.6H.etbio[,c("X6Hybrid.B73.r2","X6Hybrid.Mo17.r2")])
tmt.6H.etbio$MP3 <- rowMeans(tmt.6H.etbio[,c("X6Hybrid.B73.r3","X6Hybrid.Mo17.r3")])
tmt.6H.etbio$ttest <- ttestFun(tmt.6H.etbio, c("X6Hybrid.B73xMo17.r1", "X6Hybrid.B73xMo17.r2", "X6Hybrid.B73xMo17.r3"), c("MP1", "MP2", "MP3"))
tmt.6H.etbio$l10.ttest <- -(log10(tmt.6H.etbio$ttest))
tmt.6H.etbio$Category <- "Other"
tmt.6H.etbio$Order <- 1
tmt.6H.etbio[(tmt.6H.etbio$Gene %in% GOIs$MAT),"Category"] <- "Methionine adenosyltransferase"
tmt.6H.etbio[(tmt.6H.etbio$Gene %in% GOIs$MAT),"Order"] <- 2
tmt.6H.etbio[(tmt.6H.etbio$Gene %in% GOIs$ACS),"Category"] <- "ACC synthase"
tmt.6H.etbio[(tmt.6H.etbio$Gene %in% GOIs$ACS),"Order"] <- 3
tmt.6H.etbio[(tmt.6H.etbio$Gene %in% GOIs$ACO),"Category"] <- "ACC oxidase"
tmt.6H.etbio[(tmt.6H.etbio$Gene %in% GOIs$ACO),"Order"] <- 4
###Re-order
tmt.6H.etbio <- tmt.6H.etbio[(order(tmt.6H.etbio$Order)),]
###factor
tmt.6H.etbio$Category <- factor(tmt.6H.etbio$Category, levels = c("Other", "Methionine adenosyltransferase", "ACC synthase", "ACC oxidase"))
```
```{r setup dataframe cpm.6H.EtBio}
###Set GOIs
cpm.6H.etbio <- cpm.6H
cpm.6H.etbio$l2.B73xMo17.MP[cpm.6H.etbio$l2.B73xMo17.MP <= -2.5] <- -2.5
cpm.6H.etbio$l2.B73xMo17.MP[cpm.6H.etbio$l2.B73xMo17.MP >= 2.5] <- 2.5
cpm.6H.etbio$MP1 <- rowMeans(cpm.6H.etbio[,c("B73.2","Mo17.2")])
cpm.6H.etbio$MP2 <- rowMeans(cpm.6H.etbio[,c("B73.3","Mo17.3")])
cpm.6H.etbio$MP3 <- rowMeans(cpm.6H.etbio[,c("B73.4","Mo17.4")])
cpm.6H.etbio$ttest <- ttestFun(cpm.6H.etbio, c("B73xMo17.2", "B73xMo17.3", "B73xMo17.4"), c("MP1", "MP2", "MP3"))
cpm.6H.etbio$l10.ttest <- -(log10(cpm.6H.etbio$ttest))
cpm.6H.etbio$Category <- "Other"
cpm.6H.etbio$Order <- 1
cpm.6H.etbio[(cpm.6H.etbio$Gene %in% GOIs$MAT),"Category"] <- "Methionine adenosyltransferase"
cpm.6H.etbio[(cpm.6H.etbio$Gene %in% GOIs$MAT),"Order"] <- 2
cpm.6H.etbio[(cpm.6H.etbio$Gene %in% GOIs$ACS),"Category"] <- "ACC synthase"
cpm.6H.etbio[(cpm.6H.etbio$Gene %in% GOIs$ACS),"Order"] <- 3
cpm.6H.etbio[(cpm.6H.etbio$Gene %in% GOIs$ACO),"Category"] <- "ACC oxidase"
cpm.6H.etbio[(cpm.6H.etbio$Gene %in% GOIs$ACO),"Order"] <- 4
###Re-order
cpm.6H.etbio <- cpm.6H.etbio[(order(cpm.6H.etbio$Order)),]
###factor
cpm.6H.etbio$Category <- factor(cpm.6H.etbio$Category, levels = c("Other", "Methionine adenosyltransferase", "ACC synthase", "ACC oxidase"))
```
```{r plot tmt.cpm.6H.EtBio}
#Scatterplot - tmt
tmt.6H.etbio.vol <- ggplot(tmt.6H.etbio, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=colors.e, name="Category")+
  scale_shape_manual(values=c(1,19,19,19))+
    theme_classic()+
...
#volterplot - cpm
cpm.6H.etbio.vol <- ggplot(cpm.6H.etbio, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=colors.e, name="Category")+
  scale_shape_manual(values=c(1,19,19,19))+
    theme_classic()+
...
```{r setup dataframe tmt.SL.EtBio}
###Set GOIs
tmt.RIL.etbio <- tmt.RIL
tmt.RIL.etbio$l2.B73xMo17.MP[tmt.RIL.etbio$l2.B73xMo17.MP <= -1] <- -1
tmt.RIL.etbio$l2.B73xMo17.MP[tmt.RIL.etbio$l2.B73xMo17.MP >= 1] <- 1
tmt.RIL.etbio$MP1 <- rowMeans(tmt.RIL.etbio[,c("B73.1","Mo17.1")])
tmt.RIL.etbio$MP2 <- rowMeans(tmt.RIL.etbio[,c("B73.3","Mo17.3")])
tmt.RIL.etbio$MP3 <- rowMeans(tmt.RIL.etbio[,c("B73.4","Mo17.4")])
tmt.RIL.etbio$ttest <- ttestFun(tmt.RIL.etbio, c("BM.1", "BM.3", "BM.4", "BM.2"), c("MP1", "MP2", "MP3"))
tmt.RIL.etbio$l10.ttest <- -(log10(tmt.RIL.etbio$ttest))
tmt.RIL.etbio$Category <- "Other"
tmt.RIL.etbio$Order <- 1
tmt.RIL.etbio[(tmt.RIL.etbio$Gene %in% GOIs$MAT),"Category"] <- "Methionine adenosyltransferase"
tmt.RIL.etbio[(tmt.RIL.etbio$Gene %in% GOIs$MAT),"Order"] <- 2
tmt.RIL.etbio[(tmt.RIL.etbio$Gene %in% GOIs$ACS),"Category"] <- "ACC synthase"
tmt.RIL.etbio[(tmt.RIL.etbio$Gene %in% GOIs$ACS),"Order"] <- 3
tmt.RIL.etbio[(tmt.RIL.etbio$Gene %in% GOIs$ACO),"Category"] <- "ACC oxidase"
tmt.RIL.etbio[(tmt.RIL.etbio$Gene %in% GOIs$ACO),"Order"] <- 4
###Re-order
tmt.RIL.etbio <- tmt.RIL.etbio[(order(tmt.RIL.etbio$Order)),]
###factor
tmt.RIL.etbio$Category <- factor(tmt.RIL.etbio$Category, levels = c("Other", "Methionine adenosyltransferase", "ACC synthase", "ACC oxidase"))
```
```{r setup dataframe cpm.RIL.EtBio}
###Set GOIs
cpm.RIL.etbio <- cpm.RIL
cpm.RIL.etbio$l2.B73xMo17.MP[cpm.RIL.etbio$l2.B73xMo17.MP <= -2.5] <- -2.5
cpm.RIL.etbio$l2.B73xMo17.MP[cpm.RIL.etbio$l2.B73xMo17.MP >= 2.5] <- 2.5
cpm.RIL.etbio$MP1 <- rowMeans(cpm.RIL.etbio[,c("B73.1","Mo17.1")])
cpm.RIL.etbio$MP2 <- rowMeans(cpm.RIL.etbio[,c("B73.2","Mo17.2")])
cpm.RIL.etbio$MP3 <- rowMeans(cpm.RIL.etbio[,c("B73.3","Mo17.3")])
cpm.RIL.etbio$MP4 <- rowMeans(cpm.RIL.etbio[,c("B73.4","Mo17.4")])
cpm.RIL.etbio$ttest <- ttestFun(cpm.RIL.etbio, c("BxM.1", "BxM.2", "BxM.3", "BxM.4"), c("MP1", "MP2", "MP3", "MP4"))
cpm.RIL.etbio$l10.ttest <- -(log10(cpm.RIL.etbio$ttest))
cpm.RIL.etbio$Category <- "Other"
cpm.RIL.etbio$Order <- 1
cpm.RIL.etbio[(cpm.RIL.etbio$Gene %in% GOIs$MAT),"Category"] <- "Methionine adenosyltransferase"
cpm.RIL.etbio[(cpm.RIL.etbio$Gene %in% GOIs$MAT),"Order"] <- 2
cpm.RIL.etbio[(cpm.RIL.etbio$Gene %in% GOIs$ACS),"Category"] <- "ACC synthase"
cpm.RIL.etbio[(cpm.RIL.etbio$Gene %in% GOIs$ACS),"Order"] <- 3
cpm.RIL.etbio[(cpm.RIL.etbio$Gene %in% GOIs$ACO),"Category"] <- "ACC oxidase"
cpm.RIL.etbio[(cpm.RIL.etbio$Gene %in% GOIs$ACO),"Order"] <- 4
###Re-order
cpm.RIL.etbio <- cpm.RIL.etbio[(order(cpm.RIL.etbio$Order)),]
###factor
cpm.RIL.etbio$Category <- factor(cpm.RIL.etbio$Category, levels = c("Other", "Methionine adenosyltransferase", "ACC synthase", "ACC oxidase"))
```
```{r plot tmt.cpm.RIL.EtBio}
#Scatterplot - tmt
tmt.RIL.etbio.vol <- ggplot(tmt.RIL.etbio, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=colors.e, name="Category")+
  scale_shape_manual(values=c(1,19,19,19))+
    theme_classic()+
    ...
    coord_fixed(expand = F)
#volterplot - cpm
cpm.RIL.etbio.vol <- ggplot(cpm.RIL.etbio, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=colors.e, name="Category")+
  scale_shape_manual(values=c(1,19,19,19))+
    theme_classic()+
    ...
    theme(aspect.ratio=1)+
    coord_fixed(expand = F)
cpm.RIL.etbio.vol.legend <- get_legend(cpm.RIL.etbio.vol)
cpm.RIL.etbio.vol <- cpm.RIL.etbio.vol+theme(legend.position = "none")
```
##DN
```{r setup dataframe tmt.SL.EtBio}
###Set GOIs
tmt.DN.etbio <- tmt.DN
tmt.DN.etbio$l2.B73xMo17.MP[tmt.DN.etbio$l2.B73xMo17.MP <= -1] <- -1
tmt.DN.etbio$l2.B73xMo17.MP[tmt.DN.etbio$l2.B73xMo17.MP >= 1] <- 1
tmt.DN.etbio$MP1 <- rowMeans(tmt.DN.etbio[,c("Day.B73.r1","Day.Mo17.r1")])
tmt.DN.etbio$MP2 <- rowMeans(tmt.DN.etbio[,c("Day.B73.r2","Day.Mo17.r2")])
tmt.DN.etbio$MP3 <- rowMeans(tmt.DN.etbio[,c("Day.B73.r3","Day.Mo17.r3")])
tmt.DN.etbio$ttest <- ttestFun(tmt.DN.etbio, c("Day.BM.r1", "Day.BM.r2", "Day.BM.r3"), c("MP1", "MP2", "MP3"))
tmt.DN.etbio$l10.ttest <- -(log10(tmt.DN.etbio$ttest))
tmt.DN.etbio$Category <- "Other"
tmt.DN.etbio$Order <- 1
tmt.DN.etbio[(tmt.DN.etbio$Gene %in% GOIs$MAT),"Category"] <- "Methionine adenosyltransferase"
tmt.DN.etbio[(tmt.DN.etbio$Gene %in% GOIs$MAT),"Order"] <- 2
tmt.DN.etbio[(tmt.DN.etbio$Gene %in% GOIs$ACS),"Category"] <- "ACC synthase"
tmt.DN.etbio[(tmt.DN.etbio$Gene %in% GOIs$ACS),"Order"] <- 3
tmt.DN.etbio[(tmt.DN.etbio$Gene %in% GOIs$ACO),"Category"] <- "ACC oxidase"
tmt.DN.etbio[(tmt.DN.etbio$Gene %in% GOIs$ACO),"Order"] <- 4
###Re-order
tmt.DN.etbio <- tmt.DN.etbio[(order(tmt.DN.etbio$Order)),]
###factor
tmt.DN.etbio$Category <- factor(tmt.DN.etbio$Category, levels = c("Other", "Methionine adenosyltransferase", "ACC synthase", "ACC oxidase"))
```
```{r plot tmt.DN.EtBio}
#Scatterplot - tmt
tmt.DN.etbio.vol <- ggplot(tmt.DN.etbio, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=colors.e, name="Category")+
  scale_shape_manual(values=c(1,19,19,19))+
    theme_classic()+
    ...
    theme(aspect.ratio=1)+
    coord_fixed(expand = F)
tmt.DN.etbio.vol.legend <- get_legend(tmt.DN.etbio.vol)
tmt.DN.etbio.vol <- tmt.DN.etbio.vol+theme(legend.position = "none")
```
##Panels
```{r}
       des <- "AB
               CD
               EE"
  r<-patchwork::wrap_plots(A=tmt.SL.etbio.vol, B=tmt.6H.etbio.vol, C=tmt.RIL.etbio.vol, D=tmt.DN.etbio.vol, E=tmt.DN.etbio.vol.legend, nrow = 3, heights = c(3,3,1.2), design = des)
  ggsave(plot=r, filename =paste0(PathToPlots, "tmt.BxM.SL.etbio.all4.vol.pdf"),
          dpi=300, width=4.75, height=5) 
```
```{r}
       des <- "AB
               C#
               EE"
  r<-patchwork::wrap_plots(A=cpm.SL.etbio.vol, B=cpm.6H.etbio.vol, C=cpm.RIL.etbio.vol, E=cpm.RIL.etbio.vol.legend, nrow = 3, heights = c(3,3,1.2), design = des)
  ggsave(plot=r, filename =paste0(PathToPlots, "cpm.BxM.SL.etbio.all4.vol.pdf"),
          dpi=300, width=4.75, height=5) 
```


#Figure tmt.cpm.SL.acs.vol
...
design <- "AB
           CC"
plotpanel<-patchwork::wrap_plots(A=tmt.SL.acs.vol, B=cpm.SL.acs.vol, C=cpm.SL.acs.vol.legend,  nrow = 2, heights = c(3,2.5), design = design)
ggsave(plot=plotpanel, filename =paste0(PathToPlots, "tmt.cpm.SL.acs.vol.pdf"), dpi=300, width=4.75, height=3)
```

```{r}
EthyleneBiosynthesisEnzymes <- data.frame("Gene"=c(GOIs$SAMS, GOIs$ACS, GOIs$ACO, GOIs$acs2, GOIs$acs6, GOIs$AtACO1hom, GOIs$AtACO2hom, GOIs$AtSAM1AtSAM2hom), "Enzyme"=NA)
```
```{r}
EthyleneBiosynthesisEnzymes$Gene <- as.character(EthyleneBiosynthesisEnzymes$Gene)
EthyleneBiosynthesisEnzymes$order <- NA
EthyleneBiosynthesisEnzymes$Enzyme[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$SAMS] <- "SAMS"
EthyleneBiosynthesisEnzymes$order[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$SAMS] <- 2
EthyleneBiosynthesisEnzymes$Enzyme[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$ACS] <- "ACS"
EthyleneBiosynthesisEnzymes$order[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$ACS] <- 3
EthyleneBiosynthesisEnzymes$Enzyme[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$ACO] <- "ACO"
EthyleneBiosynthesisEnzymes$order[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$ACO] <- 4
EthyleneBiosynthesisEnzymes$Enzyme[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$acs2] <- "acs2"
EthyleneBiosynthesisEnzymes$order[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$acs2] <- 5
EthyleneBiosynthesisEnzymes$Enzyme[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$acs6] <- "acs6"
EthyleneBiosynthesisEnzymes$order[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$acs6] <- 6
EthyleneBiosynthesisEnzymes$Enzyme[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$AtACO1hom] <- "AtACO1hom"
EthyleneBiosynthesisEnzymes$order[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$AtACO1hom] <- 7
EthyleneBiosynthesisEnzymes$Enzyme[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$AtACO2hom] <- "AtACO2hom"
EthyleneBiosynthesisEnzymes$order[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$AtACO2hom] <- 8
EthyleneBiosynthesisEnzymes$Enzyme[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$AtSAM1AtSAM2hom] <- "AtSAM1AtSAM2hom"
EthyleneBiosynthesisEnzymes$order[EthyleneBiosynthesisEnzymes$Gene %in% GOIs$AtSAM1AtSAM2hom] <- 9
EthyleneBiosynthesisEnzymes <- EthyleneBiosynthesisEnzymes[order(EthyleneBiosynthesisEnzymes$order),]
write.csv(EthyleneBiosynthesisEnzymes, "EthyleneBiosynthesisEnzymes.csv", row.names = F)
EthyleneBiosynthesisEnzymes
```
...
#Figure tmt.cpm.SL.sa.vol
```{r setup dataframe tmt.SL.sa.vol}
###Set GOIs
tmt.SL.sa <- tmt.SL
tmt.SL.sa$l2.B73xMo17.MP[tmt.SL.sa$l2.B73xMo17.MP <= -1] <- -1
tmt.SL.sa$l2.B73xMo17.MP[tmt.SL.sa$l2.B73xMo17.MP >= 1] <- 1
tmt.SL.sa$MP1 <- rowMeans(tmt.SL.sa[,c("SeedlingLeaf.B73.r1","SeedlingLeaf.Mo17.r1")])
tmt.SL.sa$MP2 <- rowMeans(tmt.SL.sa[,c("SeedlingLeaf.B73.r2","SeedlingLeaf.Mo17.r2")])
tmt.SL.sa$MP3 <- rowMeans(tmt.SL.sa[,c("SeedlingLeaf.B73.r3","SeedlingLeaf.Mo17.r3")])
tmt.SL.sa$ttest <- ttestFun(tmt.SL.sa, c("SeedlingLeaf.BM.r1", "SeedlingLeaf.BM.r2", "SeedlingLeaf.BM.r3"), c("MP1", "MP2", "MP3"))
tmt.SL.sa$l10.ttest <- -(log10(tmt.SL.sa$ttest))
tmt.SL.sa$Category <- "Other"
tmt.SL.sa$Order <- 1
tmt.SL.sa[(tmt.SL.sa$Gene %in% ics),"Category"] <- "ics"
tmt.SL.sa[(tmt.SL.sa$Gene %in% ics),"Order"] <- 2
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal1),"Category"] <- "pal1"
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal1),"Order"] <- 4
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal4),"Category"] <- "pal4"
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal4),"Order"] <- 5
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal5),"Category"] <- "pal5"
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal5),"Order"] <- 6
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal6),"Category"] <- "pal6"
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal6),"Order"] <- 7
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal7),"Category"] <- "pal7"
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal7),"Order"] <- 8
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal8),"Category"] <- "pal8"
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal8),"Order"] <- 9
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal9),"Category"] <- "pal9"
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal9),"Order"] <- 10
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal10),"Category"] <- "pal10"
tmt.SL.sa[(tmt.SL.sa$Gene %in% pal10),"Order"] <- 11
###Re-order
tmt.SL.sa <- tmt.SL.sa[(order(tmt.SL.sa$Order)),]
###factor
tmt.SL.sa$Category <- factor(tmt.SL.sa$Category, levels = c("Other", "ics", "pal1", "pal4", "pal5", "pal6", "pal7","pal8", "pal9", "pal10"))
```
```{r setup dataframe cpm.SL.sa.vol}
###Set GOIs
cpm.SL.sa <- cpm.SL
cpm.SL.sa$l2.B73xMo17.MP[cpm.SL.sa$l2.B73xMo17.MP <= -2.5] <- -2.5
cpm.SL.sa$l2.B73xMo17.MP[cpm.SL.sa$l2.B73xMo17.MP >= 2.5] <- 2.5
cpm.SL.sa$MP1 <- rowMeans(cpm.SL.sa[,c("B73.1","Mo17.1")])
cpm.SL.sa$MP2 <- rowMeans(cpm.SL.sa[,c("B73.2","Mo17.2")])
cpm.SL.sa$MP3 <- rowMeans(cpm.SL.sa[,c("B73.3","Mo17.3")])
cpm.SL.sa$ttest <- ttestFun(cpm.SL.sa, c("B73xMo17.1", "B73xMo17.2", "B73xMo17.3"), c("MP1", "MP2", "MP3"))
cpm.SL.sa$l10.ttest <- -(log10(cpm.SL.sa$ttest))
cpm.SL.sa$Category <- "Other"
cpm.SL.sa$Order <- 1
cpm.SL.sa[(cpm.SL.sa$Gene %in% ics),"Category"] <- "ics"
cpm.SL.sa[(cpm.SL.sa$Gene %in% ics),"Order"] <- 2
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal1),"Category"] <- "pal1"
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal1),"Order"] <- 4
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal4),"Category"] <- "pal4"
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal4),"Order"] <- 5
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal5),"Category"] <- "pal5"
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal5),"Order"] <- 6
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal6),"Category"] <- "pal6"
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal6),"Order"] <- 7
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal7),"Category"] <- "pal7"
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal7),"Order"] <- 8
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal8),"Category"] <- "pal8"
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal8),"Order"] <- 9
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal9),"Category"] <- "pal9"
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal9),"Order"] <- 10
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal10),"Category"] <- "pal10"
cpm.SL.sa[(cpm.SL.sa$Gene %in% pal10),"Order"] <- 11
###Re-order
cpm.SL.sa <- cpm.SL.sa[(order(cpm.SL.sa$Order)),]
###factor
cpm.SL.sa$Category <- factor(cpm.SL.sa$Category, levels = c("Other", "ics", "pal1", "pal4", "pal5", "pal6", "pal7","pal8", "pal9", "pal10"))
```

```{r setup dataframe tmt.acs.sa.vol}
###Set GOIs
tmt.acs.sa <- tmt.acs
tmt.acs.sa$l2.ACS.B73[tmt.acs.sa$l2.ACS.B73 <= -1] <- -1
tmt.acs.sa$l2.ACS.B73[tmt.acs.sa$l2.ACS.B73 >= 1] <- 1
tmt.acs.sa$ttest <- ttestFun(tmt.acs.sa, c("B73.acs.B73.r1", "B73.acs.B73.r2", "B73.acs.B73.r3", "B73.acs.B73.r4", "B73.acs.B73.r5"), c("B73.acs.acs.r1", "B73.acs.acs.r2", "B73.acs.acs.r3", "B73.acs.acs.r4", "B73.acs.acs.r5"))
tmt.acs.sa$l10.ttest <- -(log10(tmt.acs.sa$ttest))
tmt.acs.sa$Category <- "Other"
tmt.acs.sa$Order <- 1
tmt.acs.sa[(tmt.acs.sa$Gene %in% ics),"Category"] <- "ics"
tmt.acs.sa[(tmt.acs.sa$Gene %in% ics),"Order"] <- 2
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal1),"Category"] <- "pal1"
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal1),"Order"] <- 4
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal4),"Category"] <- "pal4"
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal4),"Order"] <- 5
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal5),"Category"] <- "pal5"
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal5),"Order"] <- 6
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal6),"Category"] <- "pal6"
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal6),"Order"] <- 7
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal7),"Category"] <- "pal7"
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal7),"Order"] <- 8
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal8),"Category"] <- "pal8"
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal8),"Order"] <- 9
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal9),"Category"] <- "pal9"
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal9),"Order"] <- 10
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal10),"Category"] <- "pal10"
tmt.acs.sa[(tmt.acs.sa$Gene %in% pal10),"Order"] <- 11
###Re-order
tmt.acs.sa <- tmt.acs.sa[(order(tmt.acs.sa$Order)),]
###factor
tmt.acs.sa$Category <- factor(tmt.acs.sa$Category, levels = c("Other", "ics", "pal1", "pal4", "pal5", "pal6", "pal7","pal8", "pal9", "pal10"))
```
```{r plot tmt.acs.sa.vol}
tmt.acs.sa.vol <- ggplot(tmt.acs.sa, mapping=aes(x=l2.ACS.B73, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=colors.v[c(1,3,4,8,9,11:15)], name="Category")+
  scale_shape_manual(values=c(1,19,19,19,19,19,19,19,19,19,19))+
    ...
    theme(aspect.ratio=1)+
    coord_fixed(expand = F)
ggsave(paste0(PathToPlots, "tmt.acs.sa.vol.pdf"), width = 3, height = 3)
```

```{r plot tmt.cpm.SL.sa.vol}
#Scatterplot - tmt
tmt.SL.sa.vol <- ggplot(tmt.SL.sa, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=colors.v[c(1,3,4,8,9,11:15)], name="Category")+
  scale_shape_manual(values=c(1,19,19,19,19,19,19,19,19,19,19))+
    theme_classic()+
...
#volterplot - cpm
cpm.SL.sa.vol <- ggplot(cpm.SL.sa, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=colors.v[c(1,3,4,8,9,11:15)], name="Category")+
  scale_shape_manual(values=c(1,19,19,19,19,19,19,19,19,19,19))+
    theme_classic()+
    ...
    theme(aspect.ratio=1)+
    coord_fixed(expand = F)
cpm.SL.sa.vol.legend <- get_legend(cpm.SL.sa.vol)
cpm.SL.sa.vol <- cpm.SL.sa.vol+theme(legend.position = "none")

#Wrap into panel
design <- "AB
           CC"
plotpanel<-patchwork::wrap_plots(A=tmt.SL.sa.vol, B=cpm.SL.sa.vol, C=cpm.SL.sa.vol.legend,  nrow = 2, heights = c(3,1.2), design = design)
ggsave(plot=plotpanel, filename =paste0(PathToPlots, "tmt.cpm.SL.sa.vol.pdf"), dpi=300, width=4.75, height=3)
```
...
#Figure tmt.cpm.SL.yuc.vol
```{r setup dataframe tmt.SL.yuc.vol}
###Set GOIs
tmt.SL.yuc <- tmt.SL
tmt.SL.yuc$l2.B73xMo17.MP[tmt.SL.yuc$l2.B73xMo17.MP <= -1] <- -1
tmt.SL.yuc$l2.B73xMo17.MP[tmt.SL.yuc$l2.B73xMo17.MP >= 1] <- 1
tmt.SL.yuc$MP1 <- rowMeans(tmt.SL.yuc[,c("SeedlingLeaf.B73.r1","SeedlingLeaf.Mo17.r1")])
tmt.SL.yuc$MP2 <- rowMeans(tmt.SL.yuc[,c("SeedlingLeaf.B73.r2","SeedlingLeaf.Mo17.r2")])
tmt.SL.yuc$MP3 <- rowMeans(tmt.SL.yuc[,c("SeedlingLeaf.B73.r3","SeedlingLeaf.Mo17.r3")])
tmt.SL.yuc$ttest <- ttestFun(tmt.SL.yuc, c("SeedlingLeaf.BM.r1", "SeedlingLeaf.BM.r2", "SeedlingLeaf.BM.r3"), c("MP1", "MP2", "MP3"))
tmt.SL.yuc$l10.ttest <- -(log10(tmt.SL.yuc$ttest))
tmt.SL.yuc$Category <- "Other"
tmt.SL.yuc$Order <- 1
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc2),"Category"] <- "yuc2"
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc2),"Order"] <- 2
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc4),"Category"] <- "yuc4"
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc4),"Order"] <- 3
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc5),"Category"] <- "yuc5"
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc5),"Order"] <- 4
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc6),"Category"] <- "yuc6"
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc6),"Order"] <- 5
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc7),"Category"] <- "yuc7"
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc7),"Order"] <- 6
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc8),"Category"] <- "yuc8"
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc8),"Order"] <- 7
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc9),"Category"] <- "yuc9"
tmt.SL.yuc[(tmt.SL.yuc$Gene %in% yuc9),"Order"] <- 8
###Re-order
tmt.SL.yuc <- tmt.SL.yuc[(order(tmt.SL.yuc$Order)),]
###factor
tmt.SL.yuc$Category <- factor(tmt.SL.yuc$Category, levels = c("Other", "yuc2", "yuc4", "yuc5", "yuc6", "yuc7","yuc8", "yuc9"))
```
```{r setup dataframe cpm.SL.yuc.vol}
###Set GOIs
cpm.SL.yuc <- cpm.SL
cpm.SL.yuc$l2.B73xMo17.MP[cpm.SL.yuc$l2.B73xMo17.MP <= -2.5] <- -2.5
cpm.SL.yuc$l2.B73xMo17.MP[cpm.SL.yuc$l2.B73xMo17.MP >= 2.5] <- 2.5
cpm.SL.yuc$MP1 <- rowMeans(cpm.SL.yuc[,c("B73.1","Mo17.1")])
cpm.SL.yuc$MP2 <- rowMeans(cpm.SL.yuc[,c("B73.2","Mo17.2")])
cpm.SL.yuc$MP3 <- rowMeans(cpm.SL.yuc[,c("B73.3","Mo17.3")])
cpm.SL.yuc$ttest <- ttestFun(cpm.SL.yuc, c("B73xMo17.1", "B73xMo17.2", "B73xMo17.3"), c("MP1", "MP2", "MP3"))
cpm.SL.yuc$l10.ttest <- -(log10(cpm.SL.yuc$ttest))
cpm.SL.yuc$Category <- "Other"
cpm.SL.yuc$Order <- 1
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc2),"Category"] <- "yuc2"
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc2),"Order"] <- 2
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc4),"Category"] <- "yuc4"
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc4),"Order"] <- 3
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc5),"Category"] <- "yuc5"
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc5),"Order"] <- 4
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc6),"Category"] <- "yuc6"
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc6),"Order"] <- 5
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc7),"Category"] <- "yuc7"
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc7),"Order"] <- 6
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc8),"Category"] <- "yuc8"
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc8),"Order"] <- 7
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc9),"Category"] <- "yuc9"
cpm.SL.yuc[(cpm.SL.yuc$Gene %in% yuc9),"Order"] <- 8
###Re-order
cpm.SL.yuc <- cpm.SL.yuc[(order(cpm.SL.yuc$Order)),]
###factor
cpm.SL.yuc$Category <- factor(cpm.SL.yuc$Category, levels = c("Other", "yuc2", "yuc4", "yuc5", "yuc6", "yuc7","yuc8", "yuc9"))
```

```{r setup dataframe tmt.SL.yuc.vol}
###Set GOIs
tmt.acs.yuc <- tmt.acs
tmt.acs.yuc$l2.ACS.B73[tmt.acs.yuc$l2.ACS.B73 <= -1] <- -1
tmt.acs.yuc$l2.ACS.B73[tmt.acs.yuc$l2.ACS.B73 >= 1] <- 1
tmt.acs.yuc$ttest <- ttestFun(tmt.acs.yuc, c("B73.acs.B73.r1", "B73.acs.B73.r2", "B73.acs.B73.r3", "B73.acs.B73.r4", "B73.acs.B73.r5"), c("B73.acs.acs.r1", "B73.acs.acs.r2", "B73.acs.acs.r3", "B73.acs.acs.r4", "B73.acs.acs.r5"))
tmt.acs.yuc$l10.ttest <- -(log10(tmt.acs.yuc$ttest))
tmt.acs.yuc$Category <- "Other"
tmt.acs.yuc$Order <- 1
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc2),"Category"] <- "yuc2"
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc2),"Order"] <- 2
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc4),"Category"] <- "yuc4"
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc4),"Order"] <- 3
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc5),"Category"] <- "yuc5"
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc5),"Order"] <- 4
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc6),"Category"] <- "yuc6"
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc6),"Order"] <- 5
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc7),"Category"] <- "yuc7"
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc7),"Order"] <- 6
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc8),"Category"] <- "yuc8"
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc8),"Order"] <- 7
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc9),"Category"] <- "yuc9"
tmt.acs.yuc[(tmt.acs.yuc$Gene %in% yuc9),"Order"] <- 8
###Re-order
tmt.acs.yuc <- tmt.acs.yuc[(order(tmt.acs.yuc$Order)),]
###factor
tmt.acs.yuc$Category <- factor(tmt.acs.yuc$Category, levels = c("Other", "yuc2", "yuc4", "yuc5", "yuc6", "yuc7","yuc8", "yuc9"))
```
```{r plot tmt.acs.yuc.vol}
tmt.acs.yuc.vol <- ggplot(tmt.acs.yuc, mapping=aes(x=l2.ACS.B73, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=colors.v[c(1,11:15)], name="Category")+
  scale_shape_manual(values=c(1,19,19,19,19,19,19,19))+
    theme_classic()+
          ggtitle("A")+
          geom_hline(yintercept=1.30103, linetype="dashed", color = "gray50")+
          geom_vline(xintercept=0, linetype="dashed", color = "gray50")+
    ...
    theme(aspect.ratio=1)+
    coord_fixed(expand = F)
ggsave(paste0(PathToPlots, "tmt.acs.yuc.vol.pdf"), width = 3, height = 3)
```

```{r plot tmt.cpm.SL.yuc.vol}
#Scatterplot - tmt
tmt.SL.yuc.vol <- ggplot(tmt.SL.yuc, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=colors.v[c(1,11:15)], name="Category")+
  scale_shape_manual(values=c(1,19,19,19,19,19,19,19))+
    theme_classic()+
...
#volterplot - cpm
cpm.SL.yuc.vol <- ggplot(cpm.SL.yuc, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=2)+
  scale_color_manual(values=colors.v[c(1,11:15)], name="Category")+
  scale_shape_manual(values=c(1,19,19,19,19,19,19,19))+
    theme_classic()+
    ...
    theme(aspect.ratio=1)+
    coord_fixed(expand = F)
cpm.SL.yuc.vol.legend <- get_legend(cpm.SL.yuc.vol)
cpm.SL.yuc.vol <- cpm.SL.yuc.vol+theme(legend.position = "none")

#Wrap into panel
design <- "AB
           CC"
plotpanel<-patchwork::wrap_plots(A=tmt.SL.yuc.vol, B=cpm.SL.yuc.vol, C=cpm.SL.yuc.vol.legend,  nrow = 2, heights = c(3,1.2), design = design)
ggsave(plot=plotpanel, filename =paste0(PathToPlots, "tmt.cpm.SL.yuc.vol.pdf"), dpi=300, width=4.75, height=3)
```
...
#Figure tmt.BxM.SL.plastid.all4.vol
...
tmt.SL.GOIs$ttest <- ttestFun(tmt.SL.GOIs, c("SeedlingLeaf.BM.r1", "SeedlingLeaf.BM.r2", "SeedlingLeaf.BM.r3"), c("MP1", "MP2", "MP3"))
...
tmt.6H.GOIs$ttest <- ttestFun(tmt.6H.GOIs, c("X6Hybrid.B73xMo17.r1", "X6Hybrid.B73xMo17.r2", "X6Hybrid.B73xMo17.r3"), c("MP1", "MP2", "MP3"))
...
tmt.RIL.GOIs$ttest <- ttestFun(tmt.RIL.GOIs, c("BM.1", "BM.2", "BM.3", "BM.4"), c("MP1", "MP2", "MP3"))
...
tmt.DN.GOIs$ttest <- ttestFun(tmt.DN.GOIs, c("Day.BM.r1", "Day.BM.r2", "Day.BM.r3"), c("MP1", "MP2", "MP3"))
...
```{r}
 des <- "AB
         CD
         EE"
  r<-patchwork::wrap_plots(A=tmt.SL.GOIs.vol, B=tmt.6H.GOIs.vol, C=tmt.RIL.GOIs.vol, D=tmt.DN.GOIs.vol, E=tmt.DN.GOIs.vol.legend, nrow = 3, heights = c(3,3,1.2), design = des)
  ggsave(plot=r, filename =paste0(PathToPlots, "tmt.BxM.SL.plastid.all4.vol.pdf"),
          dpi=300, width=4.75, height=5)
```
...
```
```{r}
tmt.BxM.6H.entrez.nodups.trim.pv.df <- tmt.BxM.6H.entrez.nodups.trim.pv$plot.data.gene
tmt.BxM.6H.entrez.nodups.trim.pv.df <- merge.data.frame(tmt.BxM.6H.entrez.nodups.trim.pv.df, mgdb[,c("Entrez", "v4_gene_model")], by.x="kegg.names", by.y = "Entrez", all.x = T, all.y = F)
tmt.BxM.6H.entrez.nodups.trim.pv.df <- unique(tmt.BxM.6H.entrez.nodups.trim.pv.df[,c("v4_gene_model", "kegg.names", "labels", "all.mapped", "l2.B73xMo17.MP")])
```
...
tmt.acs.entrez.nodups.trim.pv <- pathview(gene.data = tmt.acs.entrez.nodups.trim, pathway.id = "zma00592", species = "zma",  bins = 20, limit = 0.5, out.suffix="tmt.acs.zma00592")
```
```{r}
tmt.acs.entrez.nodups.trim.pv.df <- tmt.acs.entrez.nodups.trim.pv$plot.data.gene
tmt.acs.entrez.nodups.trim.pv.df <- merge.data.frame(tmt.acs.entrez.nodups.trim.pv.df, mgdb[,c("Entrez", "v4_gene_model")], by.x="kegg.names", by.y = "Entrez", all.x = T, all.y = F)
tmt.acs.entrez.nodups.trim.pv.df <- unique(tmt.acs.entrez.nodups.trim.pv.df[,c("v4_gene_model", "kegg.names", "labels", "all.mapped", "l2.ACS.B73")])
```

#Supplemental Table 5
```{r}
SupTab5a <- (tmt.BxM.6H.entrez.nodups.trim.pv.df[,c("labels", "all.mapped")])
SupTab5a <- separate(SupTab5a, 'all.mapped', paste("all.mapped", 1:4, sep="."), sep=",", extra="drop")
SupTab5a <- melt(SupTab5a, id.vars="labels")
SupTab5a <- SupTab5a[,c("labels", "value")]
SupTab5a$value[SupTab5a$value==""] <- NA
SupTab5a <- na.omit(SupTab5a)
colnames(SupTab5a) <- c("labels", "Gene.entrez")

SupTab5b <- (tmt.acs.entrez.nodups.trim.pv.df[,c("labels", "all.mapped")])
SupTab5b <- separate(SupTab5b, 'all.mapped', paste("all.mapped", 1:4, sep="."), sep=",", extra="drop")
SupTab5b <- melt(SupTab5b, id.vars="labels")
SupTab5b <- SupTab5b[,c("labels", "value")]
SupTab5b$value[SupTab5b$value==""] <- NA
SupTab5b <- na.omit(SupTab5b)
colnames(SupTab5b) <- c("labels", "Gene.entrez")

SupTab5 <- merge.data.frame(SupTab5a, SupTab5b, by.x = "labels", by.y = "labels", all=T)
SupTab5 <- unique(SupTab5[,c(1,3)])
colnames(SupTab5) <- c("labels", "Gene.entrez")
SupTab5 <- merge.data.frame(SupTab5, mgdb[,c("v4_gene_model", "Entrez")],by.x="Gene.entrez", by.y="Entrez", all.x = T, all.y = F)
SupTab5 <- merge.data.frame(SupTab5, tmt.6H[,c("Gene", "l2.B73xMo17.MP")], by.x = "v4_gene_model", by.y = "Gene", all.x = T, ally.=F)
SupTab5 <- merge.data.frame(SupTab5, tmt.acs[,c("Gene", "l2.ACS.B73")], by.x = "v4_gene_model", by.y = "Gene", all.x = T, ally.=F)
write.csv(SupTab5, "SupplementalTable5.csv", row.names = F)
```
...
#re-order to group Hyb/MP values and Par/Par values
...
colnames(table1) <- c("Gene", "Accession", "PearsonCor")
table1$Category <- "Other"
table1$Category[table1$Gene %in% GOIs$plastid] <- "Plastid"
table1$Category[table1$Gene %in% GOIs$PE.ribo] <- "PE plastid ribosome"
table1$Category[table1$Gene %in% GOIs$NE.PtRibo] <- "NE plastid ribosome"
table1$Category[table1$Gene %in% GOIs$PhANGs] <- "PhANGs"
table1$Category[table1$Gene %in% GOIs$PhAPGs] <- "PhAPGs"
table1$Category[table1$Gene %in% GOIs$CytoRibo] <- "Cytosolic ribosome"
table1$Category[table1$Gene %in% GOIs$EtBio] <- "Ethylene biosynthesis"
table1$Category[table1$Gene %in% GOIs$acs2] <- "acs2"
table1$Category[table1$Gene %in% GOIs$acs6] <- "acs6"
#Re-order
table1 <- table1[(order(-table1$PearsonCor)),]
#Trim to top 1.5%
...
```{r suptab1 protein}
SupTab1a <- Reduce(function(x,y) merge(x,y,by="Accession",all=TRUE) ,list(tmt.SL[,c("Accession", "HYB.MP")], tmt.LB[,c("Accession", "HYB.MP")], tmt.6H[,c("Accession", "B73xMo17.MP")], tmt.RIL[,c("Accession", "BM.BM.MP")]))

colnames(SupTab1a) <- c("Accession", "Protein_SeedlingLeaf_BxM.MP", "Protein_LeafBlade_BxM.MP", "Protein_6Hybrids_BxM.MP", "Protein_RILhybrids_BxM.MP")
```
```{r suptab1 rna}
SupTab1b <- Reduce(function(x,y) merge(x,y,by="Gene",all=TRUE) ,list(cpm.SL[,c("Gene", "B73xMo17.MP")], cpm.LB[,c("Gene", "B73xMo17.MP")], cpm.6H[,c("Gene", "B73xMo17.MP")], cpm.RIL[,c("Gene", "B73xMo17.MP")]))

colnames(SupTab1b) <- c("Gene", "RNA_SeedlingLeaf_BxM.MP", "RNA_LeafBlade_BxM.MP", "RNA_6Hybrids_BxM.MP", "RNA_RILhybrids_BxM.MP")
```
```{r suptab1 combine protein and rna}
SupTab1a$Gene <- substr(SupTab1a$Accession, start = 1, stop = 14)
SupTab1 <- merge.data.frame(SupTab1a, SupTab1b, by.x = "Gene", by.y = "Gene", all = T)
SupTab1 <- SupTab1[order(-SupTab1$Protein_SeedlingLeaf_BxM.MP),]
```
```{r suptab1 add category column}
SupTab1$Category <- "Other"
SupTab1$Category[SupTab1$Gene %in% GOIs$plastid] <- "Plastid"
SupTab1$Category[SupTab1$Gene %in% GOIs$PE.ribo] <- "PE plastid ribosome"
SupTab1$Category[SupTab1$Gene %in% GOIs$NE.PtRibo] <- "NE plastid ribosome"
SupTab1$Category[SupTab1$Gene %in% GOIs$PhANGs] <- "PhANGs"
SupTab1$Category[SupTab1$Gene %in% GOIs$PhAPGs] <- "PhAPGs"
SupTab1$Category[SupTab1$Gene %in% GOIs$CytoRibo] <- "Cytosolic ribosome"
SupTab1$Category[SupTab1$Gene %in% GOIs$SAMS] <- "Ethylene biosynthesis - SAM synthase"
SupTab1$Category[SupTab1$Gene %in% GOIs$ACS] <- "Ethylene biosynthesis - ACC synthase"
SupTab1$Category[SupTab1$Gene %in% GOIs$ACO] <- "Ethylene biosynthesis - ACC oxidase"
SupTab1$Category[SupTab1$Gene %in% GOIs$acs2] <- "Ethylene biosynthesis - ACS2"
SupTab1$Category[SupTab1$Gene %in% GOIs$acs6] <- "Ethylene biosynthesis - ACS6"
SupTab1$Category[SupTab1$Gene %in% GOIs$AtACO1hom] <- "Ethylene biosynthesis - AtACO1 homolog"
SupTab1$Category[SupTab1$Gene %in% GOIs$AtACO2hom] <- "Ethylene biosynthesis - AtACO2 homolog"
SupTab1$Category[SupTab1$Gene %in% GOIs$AtSAM1AtSAM2hom] <- "Ethylene biosynthesis - AtSAM1/2 homolog"

write.csv(SupTab1[,c(1,2,11,3:10)], "SupplementalTable1.csv", row.names = F)
```
...
#Set GOIs
SupTab4 <- tmt.cpm.RIL.6H.Hyb2MP.cor
colnames(SupTab4) <- c("Gene", "Accession", "Protein_PearsonCor", "RNA_PearsonCor")
SupTab4$Category <- "Other"
SupTab4$Category[SupTab4$Gene %in% GOIs$plastid] <- "Plastid"
SupTab4$Category[SupTab4$Gene %in% GOIs$PE.ribo] <- "PE plastid ribosome"
SupTab4$Category[SupTab4$Gene %in% GOIs$NE.PtRibo] <- "NE plastid ribosome"
SupTab4$Category[SupTab4$Gene %in% GOIs$PhANGs] <- "PhANGs"
SupTab4$Category[SupTab4$Gene %in% GOIs$PhAPGs] <- "PhAPGs"
SupTab4$Category[SupTab4$Gene %in% GOIs$CytoRibo] <- "Cytosolic ribosome"
SupTab4$Category[SupTab4$Gene %in% GOIs$SAMS] <- "Ethylene biosynthesis - SAM synthase"
SupTab4$Category[SupTab4$Gene %in% GOIs$ACS] <- "Ethylene biosynthesis - ACC synthase"
SupTab4$Category[SupTab4$Gene %in% GOIs$ACO] <- "Ethylene biosynthesis - ACC oxidase"
SupTab4$Category[SupTab4$Gene %in% GOIs$acs2] <- "Ethylene biosynthesis - ACS2"
SupTab4$Category[SupTab4$Gene %in% GOIs$acs6] <- "Ethylene biosynthesis - ACS6"
SupTab4$Category[SupTab4$Gene %in% GOIs$AtACO1hom] <- "Ethylene biosynthesis - AtACO1 homolog"
SupTab4$Category[SupTab4$Gene %in% GOIs$AtACO2hom] <- "Ethylene biosynthesis - AtACO2 homolog"
SupTab4$Category[SupTab4$Gene %in% GOIs$AtSAM1AtSAM2hom] <- "Ethylene biosynthesis - AtSAM1/2 homolog"
#Re-order
SupTab4 <- SupTab4[(order(-SupTab4$Protein_PearsonCor)),]
write.csv(SupTab4[,c(1,2,5,3,4)], "SupplementalTable4.csv", row.names = F)
```
