...
#save
saveRDS(tmt.BxM.RIL, "tmt.BxM.RIL")
```
...
#save
saveRDS(tmt.BxM.DN.both, "tmt.BxM.DN.both")
```
...
#Remove proteins not detected in any of the samples
tmt.6H <- tmt.6H[rowSums(tmt.6H[,10:39]) > 0,]
#calculate averages
...
#Remove proteins not detected in any of the samples
tmt.RIL <- tmt.RIL[rowSums(tmt.RIL[,10:99]) > 0,]
#Calculate averages
...
saveRDS(tmt.acs.OG, "tmt.acs.OG")
saveRDS(tmt.acs.6H, "tmt.acs.6H")
saveRDS(tmt.acs.RIL, "tmt.acs.RIL")
saveRDS(tmt.acs.DN, "tmt.acs.DN")
```
#Import RNA data
```{r import RNA data}
#Original experiment
...
#save
saveRDS(cpm.BxM.RIL, "cpm.BxM.RIL")
```
```{r subset all hybrids from 6-hybrid experiment}
#subset
...
#limit to genes detected as protein
cpm.RIL.all9 <- cpm.RIL.all9[cpm.RIL.all9$Gene%in%tmt.RIL$Gene,]
#save
...
PE.other <- PE$Gene[!(PE$Gene%in%c(PE.PS.rubisco, PE.Complexes, PE.ribo, PE.rpo))]
PE <- PE$Gene
```
...
###Combine groups
```{r}
Ph <- unique(na.omit(c(KEGG.zma00195.tib.digenomic$Accession, KEGG.zma00196.tib.digenomic$Accession, CornCycLightRxns.digenomic$Gene.Accession, NE.Rubisco, PE.Complexes)))
```
```{r}
...
##GOI list
```{r}
GOIs <- list(PE.ribo, PhANGs, PhAPGs, plastid, NE.PtRibo, CytoRibo, EtBio)
names(GOIs) <- c("PE.ribo", "PhANGs", "PhAPGs", "plastid", "NE.PtRibo", "CytoRibo", "EtBio")
saveRDS(GOIs, "GOIs")
```
...
colors.e <- c("Other"="black","Ethylene biosynthesis"="#ef73ef")
...
tmt.PlastidRiboCor.Hyb2BxM <- tmt.PlastidRiboCor.Hyb2BxM[tmt.PlastidRiboCor.Hyb2BxM$Gene %in% c(GOIs$NE.PtRibo, GOIs$PE.ribo),]
#Remove gene column
...
tmt.PlastidRiboCor.Hyb2BxM$height <- c(rep(1, 51), rep(0.885337506, 51), rep(0.946133214, 51), rep(0.97653106, 51), rep(0.87393831, 51), rep(0.938533751, 51), rep(0.942333482, 51), rep(0.991729996, 51), rep(0.976531068, 51), rep(1, 51), rep(1.05519671, 51), rep(0.924251321, 51), rep(1.031708749, 51), rep(1.055196712, 51), rep(0.897240164, 51))
...
ggsave(paste0(PathToPlots, "SupplementalFigureS7.pdf"), width = 4.75, height = 4.5)
```
#Figure 2
```{r}
#subset RIL hybrids
...
#Trim to proteins detected in both experiments
tmt.PlastidRiboCor.Hyb2MP <- na.omit(tmt.RIL.6H.Hyb2MP)
#trim to plastid ribosomal proteins only
tmt.PlastidRiboCor.Hyb2MP <- tmt.PlastidRiboCor.Hyb2MP[tmt.PlastidRiboCor.Hyb2MP$Gene %in% c(GOIs$NE.PtRibo, GOIs$PE.ribo),]
#Remove gene column
...
tmt.PlastidRiboCor.Hyb2MP$height <- c(rep(1.169374, 51), rep(1.106888, 51), rep(1.238806, 51), rep(1.220903, 51), rep(1.144279, 51), rep(1.148837, 51), rep(1.206813, 51), rep(1.219626, 51), rep(1.256724, 51), rep(1.284798, 51), rep(1.355715, 51), rep(1.235964, 51), rep(1.362016, 51), rep(1.403358, 51), rep(1.220447, 51))
...
ggplot(data=tmt.PlastidRiboCor.Hyb2MP, mapping = aes(x=height, y=value, fill=variable))+
  ...
  stat_summary(fun.data=mean_cl_normal, show.legend = F)
ggsave(paste0(PathToPlots, "Figure2.pdf"), width = 4.75, height = 4.5)
```
#Figure S8
```{r setup dataframe S8a}
#combine RIL and 6 hybrids
cpm.RIL.6H.Hyb2MP <- merge.data.frame(cpm.RIL.all9[,c(1,17:25)], cpm.6H.all6[,c(1,12:17)], by.x = "Gene", by.y = "Gene", all.x = T, all.y = T)
...
cpm.PlastidRiboCor.hyb2MP$height <- c(rep(1.169374, 41), rep(1.106888, 41), rep(1.238806, 41), rep(1.220903, 41), rep(1.144279, 41), rep(1.148837, 41), rep(1.206813, 41), rep(1.219626, 41), rep(1.256724, 41), rep(1.284798, 41), rep(1.355715, 41), rep(1.235964, 41), rep(1.362016, 41), rep(1.403358, 41), rep(1.220447, 41))
#calculate averages and linear model
...
FigS8a <- ggplot(data=cpm.PlastidRiboCor.hyb2MP, mapping = aes(x=height, y=value, fill=variable))+
  ...
  labs(caption =  bquote(R^2 ~ "=" ~ .(cpm.PlastidRiboCor.hyb2MP.split.avg.lm.rsquared)))+
  ggtitle("A")+
  theme(text = element_text(size = 10), plot.title = element_text(vjust = -3.5, hjust = -0.485))+
...
tmt.BxM.OG.lims.etbio[(tmt.BxM.OG.lims.etbio$Gene %in% GOIs$EtBio),"Category"] <- "Ethylene biosynthesis"
tmt.BxM.OG.lims.etbio[(tmt.BxM.OG.lims.etbio$Gene %in% GOIs$EtBio),"Order"] <- 2
###Re-order
tmt.BxM.OG.lims.etbio <- tmt.BxM.OG.lims.etbio[(order(tmt.BxM.OG.lims.etbio$Order)),]
###factor
tmt.BxM.OG.lims.etbio$Category <- factor(tmt.BxM.OG.lims.etbio$Category, levels = c("Other", "Ethylene biosynthesis"))
###Scatterplot
