...
#Import protein data
tmt <- read.csv(paste0(PathToData,"SupplementalTable5.csv"), stringsAsFactors = F)
...
#6-hybrid experiment
cpm.6H <- read.csv(paste0(PathToData, "SupplementalTable6.csv"), stringsAsFactors = F)
#RIL experiment
cpm.RIL <- read.csv(paste0(PathToData, "SupplementalTable7.csv"), stringsAsFactors = F)
```
...
##GOI list
```{r make GOIs list}
GOIs <- list(PE.ribo, PhANGs, PhAPGs, plastid, NE.PtRibo, CytoRibo, EtBio, acs2, acs6, AtACO1hom, AtACO2hom, AtSAM1AtSAM2hom, SAMS, ACS, ACO, CarbonFixation, TPR, PTAC, Oxidoreductase, BiosynSecMet, Protease, AlphaLinAcidMet, ProteinBioSyn)
names(GOIs) <- c("PE.ribo", "PhANGs", "PhAPGs", "plastid", "NE.PtRibo", "CytoRibo", "EtBio", "acs2",  "acs6", "AtACO1hom", "AtACO2hom", "AtSAM1AtSAM2hom", "SAMS", "ACS", "ACO", "CarbonFixation", "TPR", "PTAC", "Oxidoreductase", "BiosynSecMet", "Protease", "AlphaLinAcidMet", "ProteinBioSyn")
saveRDS(GOIs, "GOIs")
```
...
```
```{r plotting colors}
colors.p <- c("Other"="grey70", "Plastid-localized"="black", "Cytosolic ribosome"="#EF5107", "PE plastid ribosome"="#ef73ef", "NE plastid ribosome"="#a742da", "PhAPGs"="#7ae281", "PhANGs"="#009fa7", "Ethylene biosynthesis"="#fab5bd")
...
colors.pe <- c("Other"="black", "PE plastid ribosome"="#ef73ef", "NE plastid ribosome"="#a742da", "PhAPGs"="#7ae281", "PhANGs"="#009fa7", "Ethylene biosynthesis"="#fab5bd", "Cytosolic ribosome"="#EF5107")
```
#Figure 1
...
#Figure S3
```{r setup dataframe cpm.SL.figs3}
#copy
...
#bind
cpm.SL.figs3 <- rbind(cpm.SL.Hyb2HP,cpm.SL.Hyb2MP)
cpm.SL.figs3 <- rbind(cpm.SL.figs3,cpm.SL.Par2Par)
```
```{r plot figs3}
 plot<-unique(cpm.SL.figs3[,"plot"])
  plotList<-vector(mode = "list", length = length(plot))
  plotLab<-unique(cpm.SL.figs3[,"plot"])
  for(i in 1:length(plot)){
    df<-cpm.SL.figs3[which(cpm.SL.figs3[,"plot"]==plot[i]),]
    ###Set limits
df.lims <- df
df.lims$l2.ratio[df.lims$l2.ratio <= -1] <- -1
df.lims$l2.ratio[df.lims$l2.ratio >= 1] <- 1
...
#Figure S2
```{r setup dataframe cpm.SL.figs2}
#copy
cpm.SL.Hyb2HP <- cpm.SL
cpm.SL.Par2Par <- cpm.SL
cpm.SL.Hyb2MP <- cpm.SL
...
colnames(cpm.SL.Par2Par)[3:4] <- c("l2.ratio", "l10.ttest")
#bind
cpm.SL.figs2 <- rbind(cpm.SL.Hyb2HP,cpm.SL.Hyb2MP)
cpm.SL.figs2 <- rbind(cpm.SL.figs2,cpm.SL.Par2Par)
```
```{r plot figS2}
 plot<-unique(cpm.SL.figs2[,"plot"])
  plotList<-vector(mode = "list", length = length(plot))
  plotLab<-unique(cpm.SL.figs2[,"plot"])
  for(i in 1:length(plot)){
    df<-cpm.SL.figs2[which(cpm.SL.figs2[,"plot"]==plot[i]),]
    ###Set limits
df.lims <- df
df.lims$l2.ratio[df.lims$l2.ratio <= -1] <- -1
df.lims$l2.ratio[df.lims$l2.ratio >= 1] <- 1
...
cpm.RIL.6H.Hyb2MP.Hyb2HP.cor[(cpm.RIL.6H.Hyb2MP.Hyb2HP.cor$Gene %in% TPR),"Category"] <- "TPR"
cpm.RIL.6H.Hyb2MP.Hyb2HP.cor[(cpm.RIL.6H.Hyb2MP.Hyb2HP.cor$Gene %in% PTAC),"Category"] <- "PTAC"
cpm.RIL.6H.Hyb2MP.Hyb2HP.cor[(cpm.RIL.6H.Hyb2MP.Hyb2HP.cor$Gene %in% GOIs$PhANGs),"Category"] <- "PhANGs"
cpm.RIL.6H.Hyb2MP.Hyb2HP.cor[(cpm.RIL.6H.Hyb2MP.Hyb2HP.cor$Gene %in% ProteinBioSyn),"Category"] <- "Protein biosynthesis"
cpm.RIL.6H.Hyb2MP.Hyb2HP.cor[(cpm.RIL.6H.Hyb2MP.Hyb2HP.cor$Gene %in% CarbonFixation),"Category"] <- "Carbon fixation"
cpm.RIL.6H.Hyb2MP.Hyb2HP.cor[(cpm.RIL.6H.Hyb2MP.Hyb2HP.cor$Gene %in% Oxidoreductase),"Category"] <- "Oxidoreductase"
cpm.RIL.6H.Hyb2MP.Hyb2HP.cor[(cpm.RIL.6H.Hyb2MP.Hyb2HP.cor$Gene %in% Protease),"Category"] <- "Protease"
cpm.RIL.6H.Hyb2MP.Hyb2HP.cor[(cpm.RIL.6H.Hyb2MP.Hyb2HP.cor$Gene %in% BiosynSecMet),"Category"] <- "Biosynthesis of secondary metabolites"
cpm.RIL.6H.Hyb2MP.Hyb2HP.cor[(cpm.RIL.6H.Hyb2MP.Hyb2HP.cor$Gene %in% AlphaLinAcidMet),"Category"] <- "Alpha-linoleic acid metabolism"
...
ggplot(cpm.RIL.6H.Hyb2MP.Hyb2HP.cor, mapping=aes(y=Category, x=cor, fill = stat(x)))+
  ...
  xlab("Pearson correlation")+
  xlim(-1,1)
ggsave(filename =paste0(PathToPlots, "FigureS4.pdf"), dpi=300, width=3.4252, height=4)
```
...
tmt.acs.lims$l2.A682xB73.MP[tmt.acs.lims$l2.A682xB73.MP <= -1] <- -1
tmt.acs.lims$l2.A682xB73.MP[tmt.acs.lims$l2.A682xB73.MP >= 1] <- 1

tmt.acs.lims$l2.ACS.B73[tmt.acs.lims$l2.ACS.B73 <= -1] <- -1
tmt.acs.lims$l2.ACS.B73[tmt.acs.lims$l2.ACS.B73 >= 1] <- 1
###Set GOIs
...
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$AtACO1hom),"Category"] <- "AtACO1 homolog"
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$AtACO1hom),"Order"] <- 7
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$AtACO2hom),"Category"] <- "AtACO2 homolog"
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$AtACO2hom),"Order"] <- 8
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$AtSAM1AtSAM2hom),"Category"] <- "AtSAM1/AtSAM2 homolog"
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$AtSAM1AtSAM2hom),"Order"] <- 9
...
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$AtACO1hom),"Category"] <- "AtACO1 homolog"
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$AtACO1hom),"Order"] <- 7
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$AtACO2hom),"Category"] <- "AtACO2 homolog"
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$AtACO2hom),"Order"] <- 8
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$AtSAM1AtSAM2hom),"Category"] <- "AtSAM1/AtSAM2 homolog"
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$AtSAM1AtSAM2hom),"Order"] <- 9
...
tmt.SL.acs.vol <- ggplot(tmt.SL.acs, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
  ...
  scale_shape_manual(values=c("Other"= 1, "SAMS"= 22, "ACS"= 23, "ACO"= 24,"ZmACS2"= 18,"ZmACS6"= 18,"AtACO1 homolog"=17,"AtACO2 homolog"=17, "AtSAM1/AtSAM2 homolog"=15))+
    theme_classic()+
          ...
          geom_vline(xintercept=0, linetype="dashed", color = "gray50")+
          theme(axis.text=element_text(size=10), legend.text=element_text(size=10), axis.title = element_text(size = 10),
                legend.position="none", legend.title = element_blank(),  plot.title = element_text(size=10, hjust = -.1))+
    xlab("log2 BxM/MP")+
    ylab("-log10(p-value)")+
    theme(aspect.ratio=1)+
    coord_fixed(expand = F)
...
cpm.SL.acs.vol <- ggplot(cpm.SL.acs, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
  ...
  scale_shape_manual(values=c("Other"= 1, "SAMS"= 22, "ACS"= 23, "ACO"= 24,"ZmACS2"= 18,"ZmACS6"= 18,"AtACO1 homolog"=17,"AtACO2 homolog"=17, "AtSAM1/AtSAM2 homolog"=15))+
    theme_classic()+
          ...
          geom_vline(xintercept=0, linetype="dashed", color = "gray50")+
          theme(axis.text=element_text(size=10), legend.text=element_text(size=10),axis.title = element_text(size = 10),
                legend.position="bottom", legend.title = element_blank(),  plot.title = element_text(size=10, hjust = -.1))+
...
design <- "AB
           CC"
plotpanel<-patchwork::wrap_plots(A=tmt.SL.acs.vol, B=cpm.SL.acs.vol, C=cpm.SL.acs.vol.legend,  nrow = 2, heights = c(3,2), design = design)
ggsave(plot=plotpanel, filename =paste0(PathToPlots, "FigureS6.pdf"), dpi=300, width=4.488189, height=3.5)
```
...
```{r setp dataframe ST1}
ST1 <- tmt.SL[,c("Gene", "Accession", "Protein", "B73xMo17.MP", "B73.Mo17")]
colnames(ST1) <- c("Gene", "Accession", "Protein", "BxM.Midparent", "B73.Mo17")
#Add category column
ST1$Category <- "Other"
ST1$Category[ST1$Gene %in% GOIs$plastid] <- "Plastid"
ST1$Category[ST1$Gene %in% GOIs$PE.ribo] <- "PE plastid ribosome"
ST1$Category[ST1$Gene %in% GOIs$NE.PtRibo] <- "NE plastid ribosome"
ST1$Category[ST1$Gene %in% GOIs$PhANGs] <- "PhANGs"
ST1$Category[ST1$Gene %in% GOIs$PhAPGs] <- "PhAPGs"
ST1$Category[ST1$Gene %in% GOIs$CytoRibo] <- "Cytosolic ribosome"
ST1$Category[ST1$Gene %in% GOIs$SAMS] <- "Ethylene biosynthesis - SAM synthase"
ST1$Category[ST1$Gene %in% GOIs$ACS] <- "Ethylene biosynthesis - ACC synthase"
ST1$Category[ST1$Gene %in% GOIs$ACO] <- "Ethylene biosynthesis - ACC oxidase"
ST1$Category[ST1$Gene %in% GOIs$acs2] <- "Ethylene biosynthesis - ACS2"
ST1$Category[ST1$Gene %in% GOIs$acs6] <- "Ethylene biosynthesis - ACS6"
ST1$Category[ST1$Gene %in% GOIs$AtACO1hom] <- "Ethylene biosynthesis - AtACO1 homolog"
ST1$Category[ST1$Gene %in% GOIs$AtACO2hom] <- "Ethylene biosynthesis - AtACO2 homolog"
ST1$Category[ST1$Gene %in% GOIs$AtSAM1AtSAM2hom] <- "Ethylene biosynthesis - AtSAM1/2 homolog"

write.csv(ST1, paste0(PathToTables,"SupplementalTable1.csv"), row.names = F)
```

#Supplemental Table 4
```{r setup dataframe ST4}
#combine RIL and 6 hybrids Protein
tmt.RIL.6H.Hyb2MP <- merge.data.frame(tmt.RIL[,c(6,127:135)], tmt.6H[,c(6,54:59)], by.x = "Accession", by.y = "Accession", all = F)
...
#Set GOIs
SupT4 <- tmt.cpm.RIL.6H.Hyb2MP.cor
colnames(SupT4) <- c("Gene", "Accession", "Protein_PearsonCor", "RNA_PearsonCor")
SupT4$Category <- "Other"
SupT4$Category[SupT4$Gene %in% GOIs$plastid] <- "Plastid"
SupT4$Category[SupT4$Gene %in% GOIs$CarbonFixation] <- "Carbon fixation"
SupT4$Category[SupT4$Gene %in% GOIs$TPR] <- "TPR"
SupT4$Category[SupT4$Gene %in% GOIs$PTAC] <- "PTAC"
SupT4$Category[SupT4$Gene %in% GOIs$Oxidoreductase] <- "Oxidoreductase"
SupT4$Category[SupT4$Gene %in% GOIs$Protease] <- "Protease"
SupT4$Category[SupT4$Gene %in% GOIs$AlphaLinAcidMet] <- "Alpha-linoleic acid metabolism"
SupT4$Category[SupT4$Gene %in% GOIs$BiosynSecMet] <- "Biosynthesis of secondary metabolites"
SupT4$Category[SupT4$Gene %in% GOIs$PE.ribo] <- "PE plastid ribosome"
SupT4$Category[SupT4$Gene %in% GOIs$NE.PtRibo] <- "NE plastid ribosome"
SupT4$Category[SupT4$Gene %in% GOIs$PhANGs] <- "PhANGs"
SupT4$Category[SupT4$Gene %in% GOIs$PhAPGs] <- "PhAPGs"
SupT4$Category[SupT4$Gene %in% GOIs$CytoRibo] <- "Cytosolic ribosome"
SupT4$Category[SupT4$Gene %in% GOIs$SAMS] <- "Ethylene biosynthesis - SAM synthase"
SupT4$Category[SupT4$Gene %in% GOIs$ACS] <- "Ethylene biosynthesis - ACC synthase"
SupT4$Category[SupT4$Gene %in% GOIs$ACO] <- "Ethylene biosynthesis - ACC oxidase"
SupT4$Category[SupT4$Gene %in% GOIs$acs2] <- "Ethylene biosynthesis - ACS2"
SupT4$Category[SupT4$Gene %in% GOIs$acs6] <- "Ethylene biosynthesis - ACS6"
SupT4$Category[SupT4$Gene %in% GOIs$AtACO1hom] <- "Ethylene biosynthesis - AtACO1 homolog"
SupT4$Category[SupT4$Gene %in% GOIs$AtACO2hom] <- "Ethylene biosynthesis - AtACO2 homolog"
SupT4$Category[SupT4$Gene %in% GOIs$AtSAM1AtSAM2hom] <- "Ethylene biosynthesis - AtSAM1/2 homolog"
#Re-order
SupT4 <- SupT4[(order(-SupT4$Protein_PearsonCor)),]

write.csv(SupT4[,c(1,2,5,3,4)], paste0(PathToTables,"SupplementalTable4.csv"), row.names = F)
```
#Supplemental Table 8
```{r Setup dataframe ST8}
ST8 <- as.data.frame(unlist(GOIs, use.names = F))
colnames(ST8) <- "Accession"
ST8$Accession <- as.character(ST8$Accession)
ST8$Group <- NA
ST8$Source <- NA
ST8[(ST8$Accession %in% GOIs$plastid),"Group"] <- "Plastid-localized"
ST8[(ST8$Accession %in% GOIs$plastid),"Source"] <- "Friso et al. (2010)"
ST8[(ST8$Accession %in% GOIs$PhANGs),"Group"] <- "PhANGs"
ST8[(ST8$Accession %in% KEGG.zma00195.digenomic$Accession),"Source"] <- "KEGG zma00195"
ST8[(ST8$Accession %in% KEGG.zma00196$Accession),"Source"] <- "KEGG zma00196"
ST8[(ST8$Accession %in% CornCycLightRxns.digenomic$Gene.Accession),"Source"] <- "CornCyc Photosynthesis Light Reactions"
ST8[(ST8$Accession %in% NE.Rubisco),"Source"] <- "MaizeGDB"
ST8[(ST8$Accession %in% GOIs$PhAPGs),"Group"] <- "PhAPGs"
ST8[(ST8$Accession %in% GOIs$PhAPGs),"Source"] <- "NCBI"
ST8[(ST8$Accession %in% GOIs$NE.PtRibo),"Group"] <- "NE plastid ribosome"
ST8[(ST8$Accession %in% GOIs$NE.PtRibo),"Source"] <- "NCBI and maize-GAMER GO"
ST8[(ST8$Accession %in% GOIs$PE.ribo),"Group"] <- "PE plastid ribosome"
ST8[(ST8$Accession %in% GOIs$PE.ribo),"Source"] <- "NCBI"
ST8[(ST8$Accession %in% GOIs$CytoRibo),"Group"] <- "Cytosolic ribosome"
ST8[(ST8$Accession %in% GOIs$CytoRibo),"Source"] <- "NCBI"
ST8[(ST8$Accession %in% GOIs$SAMS),"Group"] <- "SAM synthase"
ST8[(ST8$Accession %in% GOIs$SAMS),"Source"] <- "CornCyc 2.5.1.6"
ST8[(ST8$Accession %in% GOIs$ACS),"Group"] <- "ACC synthase"
ST8[(ST8$Accession %in% GOIs$ACS),"Source"] <- "CornCyc 4.4.1.14"
ST8[(ST8$Accession %in% GOIs$ACO),"Group"] <- "ACC oxidase"
ST8[(ST8$Accession %in% GOIs$ACO),"Source"] <- "CornCyc 1.14.17.4"
ST8[(ST8$Accession %in% GOIs$acs2),"Group"] <- "ACS2"
ST8[(ST8$Accession %in% GOIs$acs2),"Source"] <- "MaizeGDB"
ST8[(ST8$Accession %in% GOIs$acs6),"Group"] <- "ACS6"
ST8[(ST8$Accession %in% GOIs$acs6),"Source"] <- "MaizeGDB"
ST8[(ST8$Accession %in% GOIs$AtACO1hom),"Group"] <- "AtACO1 homolog"
ST8[(ST8$Accession %in% GOIs$AtACO1hom),"Source"] <- "BLAST"
ST8[(ST8$Accession %in% GOIs$AtACO2hom),"Group"] <- "AtACO2 homolog"
ST8[(ST8$Accession %in% GOIs$AtACO2hom),"Source"] <- "BLAST"
ST8[(ST8$Accession %in% GOIs$AtSAM1AtSAM2hom),"Group"] <- "AtSAM1/2 homolog"
ST8[(ST8$Accession %in% GOIs$AtSAM1AtSAM2hom),"Source"] <- "BLAST"
ST8[(ST8$Accession %in% GOIs$CarbonFixation),"Group"] <- "Carbon fixation"
ST8[(ST8$Accession %in% GOIs$CarbonFixation),"Source"] <- "KEGG zma00710"
ST8[(ST8$Accession %in% GOIs$TPR),"Group"] <- "TPR"
ST8[(ST8$Accession %in% GOIs$TPR),"Source"] <- "NCBI"
ST8[(ST8$Accession %in% GOIs$PTAC),"Group"] <- "PTAC"
ST8[(ST8$Accession %in% GOIs$PTAC),"Source"] <- "NCBI"
ST8[(ST8$Accession %in% GOIs$Oxidoreductase),"Group"] <- "Oxidoreductase"
ST8[(ST8$Accession %in% GOIs$Oxidoreductase),"Source"] <- "GAMER GO:0016491"
ST8[(ST8$Accession %in% GOIs$BiosynSecMet),"Group"] <- "Biosynthesis of secondary metabolites"
ST8[(ST8$Accession %in% GOIs$BiosynSecMet),"Source"] <- "KEGG zma01110"
ST8[(ST8$Accession %in% GOIs$Protease),"Group"] <- "Protease"
ST8[(ST8$Accession %in% GOIs$Protease),"Source"] <- "GAMER GO:0008233"
ST8[(ST8$Accession %in% GOIs$AlphaLinAcidMet),"Group"] <- "Alpha-linoleic acid metabolism"
ST8[(ST8$Accession %in% GOIs$AlphaLinAcidMet),"Source"] <- "KEGG zma00592"
ST8[(ST8$Accession %in% GOIs$ProteinBioSyn),"Group"] <- "Protein biosynthesis"
ST8[(ST8$Accession %in% GOIs$ProteinBioSyn),"Source"] <- "NCBI"

ST8 <- ST8[order(ST8$Group),]

write.csv(ST8, paste0(PathToTables,"SupplementalTable8.csv"), row.names = F)
```
