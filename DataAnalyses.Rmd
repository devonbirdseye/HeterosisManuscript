...
colors.pe <- c("Other"="black", "Plastid-localized"="black", "PE plastid ribosome"="#ef73ef", "NE plastid ribosome"="#a742da", "PhAPGs"="#7ae281", "PhANGs"="#009fa7", "Ethylene biosynthesis"="#fab5bd", "Cytosolic ribosome"="#EF5107")

colors.c <- c("Other"="black", "Cytosolic ribosome"="#EF5107", "NE plastid ribosome"="#ff0000", "PhANGs"="#3447ff")

colors.pc <- c("Other"="black", "PE plastid ribosome"="#ef73ef", "NE plastid ribosome"="#a742da", "PhAPGs"="#7ae281", "PhANGs"="#009fa7", "Cytosolic ribosome"="#EF5107")
```
#Figure 1
...
#Figure 3
```{r tmt.RIL.6H.Hyb2MP.cor.densities}
...
tmt.RIL.6H.Hyb2MP.Hyb2HP.cor.plot <- ggplot(tmt.RIL.6H.Hyb2MP.Hyb2HP.cor, mapping=aes(y=Category, x=cor, fill = stat(x)))+
...
cpm.RIL.6H.Hyb2MP.Hyb2HP.cor.plot <- ggplot(cpm.RIL.6H.Hyb2MP.Hyb2HP.cor, mapping=aes(y=Category, x=cor, fill = stat(x)))+
  geom_density_ridges_gradient(alpha=0.7) +
   scale_fill_viridis_c(option = "B")+
  theme_ridges() +
  ...
  xlab("Pearson correlation")+
  xlim(-1,1)
cpm.RIL.6H.Hyb2MP.Hyb2HP.cor.plot
...
```
#Figure S5
```{r setup dataframe cpm.BxM.zma00592}
#merge with mdb to get entrez 
...
#make new dataframe with just l2(BM/MP) values and Entrez as rownames
tmt.BxM.6H.entrez.nodups.trim <- data.frame("l2.A682xB73.MP"=tmt.BxM.6H.entrez.nodups$l2.A682xB73.MP)
rownames(tmt.BxM.6H.entrez.nodups.trim) <- tmt.BxM.6H.entrez.nodups$Entrez
```
```{r plot tmt.BxM.6H.zma00592}
tmt.BxM.6H.entrez.nodups.trim.pv <- pathview(gene.data = tmt.BxM.6H.entrez.nodups.trim, pathway.id = "zma00592", species = "zma",  bins = 20, limit = 0.5, out.suffix="tmt.A682B.BxM")
```
##B
...
```{r ST1 protein}
ST1a <- Reduce(function(x,y) merge(x,y,by="Accession",all=TRUE) ,list(tmt.SL[,c("Accession", "B73xMo17.MP")], tmt.LB[,c("Accession", "B73xMo17.MP")], tmt.6H[,c("Accession", "B73xMo17.MP")], tmt.RIL[,c("Accession", "BM.BM.MP")]))

colnames(ST1a) <- c("Accession", "Protein_SeedlingLeaf_BxM.MP", "Protein_LeafBlade_BxM.MP", "Protein_6Hybrids_BxM.MP", "Protein_RILhybrids_BxM.MP")
```
```{r ST1 rna}
ST1b <- Reduce(function(x,y) merge(x,y,by="Gene",all=TRUE) ,list(cpm.SL[,c("Gene", "B73xMo17.MP")], cpm.LB[,c("Gene", "B73xMo17.MP")], cpm.6H[,c("Gene", "B73xMo17.MP")], cpm.RIL[,c("Gene", "B73xMo17.MP")]))

colnames(ST1b) <- c("Gene", "RNA_SeedlingLeaf_BxM.MP", "RNA_LeafBlade_BxM.MP", "RNA_6Hybrids_BxM.MP", "RNA_RILhybrids_BxM.MP")
```
```{r ST1 combine protein and rna}
ST1a$Gene <- substr(ST1a$Accession, start = 1, stop = 14)
ST1 <- merge.data.frame(ST1a, ST1b, by.x = "Gene", by.y = "Gene", all = T)
ST1 <- ST1[order(-ST1$Protein_SeedlingLeaf_BxM.MP),]
```
```{r ST1 add category column}
ST1$Category <- "Other"
...
write.csv(ST1[,c(1,2,11,3:10)], paste0(PathToTables,"SupplementalTable1.csv"), row.names = F)
```
...
GOI.df[(GOI.df$Accession %in% GOIs$CarbonFixation),"Group"] <- "Carbon fixation"
GOI.df[(GOI.df$Accession %in% GOIs$CarbonFixation),"Source"] <- "KEGG zma00710"
GOI.df[(GOI.df$Accession %in% GOIs$TPR),"Group"] <- "TPR"
GOI.df[(GOI.df$Accession %in% GOIs$TPR),"Source"] <- "NCBI"
GOI.df[(GOI.df$Accession %in% GOIs$PTAC),"Group"] <- "PTAC"
GOI.df[(GOI.df$Accession %in% GOIs$PTAC),"Source"] <- "NCBI"
GOI.df[(GOI.df$Accession %in% GOIs$Oxidoreductase),"Group"] <- "Oxidoreductase"
GOI.df[(GOI.df$Accession %in% GOIs$Oxidoreductase),"Source"] <- "GAMER GO:0016491"
GOI.df[(GOI.df$Accession %in% GOIs$BiosynSecMet),"Group"] <- "Biosynthesis of secondary metabolites"
GOI.df[(GOI.df$Accession %in% GOIs$BiosynSecMet),"Source"] <- "KEGG zma01110"
GOI.df[(GOI.df$Accession %in% GOIs$Protease),"Group"] <- "Protease"
GOI.df[(GOI.df$Accession %in% GOIs$Protease),"Source"] <- "GAMER GO:0008233"
GOI.df[(GOI.df$Accession %in% GOIs$AlphaLinAcidMet),"Group"] <- "Alpha-linoleic acid metabolism"
GOI.df[(GOI.df$Accession %in% GOIs$AlphaLinAcidMet),"Source"] <- "KEGG zma00592"
GOI.df[(GOI.df$Accession %in% GOIs$ProteinBioSyn),"Group"] <- "Protein biosynthesis"
GOI.df[(GOI.df$Accession %in% GOIs$ProteinBioSyn),"Source"] <- "NCBI"

GOI.df <- GOI.df[order(GOI.df$Group),]

write.csv(GOI.df, paste0(PathToTables,"SupplementalTable8.csv"), row.names = F)
```
