...
```
```{r plotting colors}
colors.p <- c("Other"="#D8D8D8", "Plastid-localized"="black", "Cytosolic ribosome"="#B50000", "PE plastid ribosome"="#E5A3FF", "NE plastid ribosome"="#6A00EA", "PhAPGs"="#81E401", "PhANGs"="#008275", "Ethylene biosynthesis"="#EF8F6Dvv")
...
colors.pe <- c("Other"="black", "Cytosolic ribosome"="#B50000", "PE plastid ribosome"="#E5A3FF", "NE plastid ribosome"="#6A00EA", "PhAPGs"="#81E401", "PhANGs"="#008275", "Ethylene biosynthesis"="#EF8F6D")
```
#Figure 1
...
```{r}
ST5a <- tmt.SL[tmt.SL$Gene %in% GOIs$EtBio,c("Gene", "Accession", "Protein", "B73xMo17.MP", "ttest.MP")]
colnames(ST5a) <- c("Gene", "Accession", "Protein", "BxM.Midparent", "p-value")
ST5a$Expression <- "Protein"
ST5b <- cpm.SL[cpm.SL$Gene %in% GOIs$EtBio,c("Gene", "B73xMo17.MP", "ttest.MP")]
colnames(ST5b) <- c("Gene", "BxM.Midparent", "p-value")
ST5b$Expression <- "RNA"
ST5b$Accession <- ""
ST5b$Protein <- ""
ST5 <- rbind(ST5a, ST5b)
ST5$Category <- "Other"
ST5[(ST5$Gene %in% GOIs$SAMS),"Category"] <- "SAMS"
ST5[(ST5$Gene %in% GOIs$ACS),"Category"] <- "ACS"
ST5[(ST5$Gene %in% GOIs$ACO),"Category"] <- "ACO"
ST5[(ST5$Gene %in% GOIs$acs2),"Category"] <- "ZmACS2"
ST5[(ST5$Gene %in% GOIs$acs6),"Category"] <- "ZmACS6"
ST5[(ST5$Gene %in% GOIs$AtACO1hom),"Category"] <- "AtACO1 homolog"
ST5[(ST5$Gene %in% GOIs$AtACO2hom),"Category"] <- "AtACO2 homolog"
ST5[(ST5$Gene %in% GOIs$AtSAM1AtSAM2hom),"Category"] <- "AtSAM1/AtSAM2 homolog"
ST5 <- ST5[order(ST5$Category, ST5$Gene, ST5$Expression),]
write.csv(ST5, paste0(PathToTables,"SupplementalTable5.csv"), row.names = F)
```

#Supplemental Table 9
```{r Setup dataframe ST9}
ST9 <- as.data.frame(unlist(GOIs, use.names = F))
colnames(ST9) <- "Accession"
ST9$Accession <- as.character(ST9$Accession)
ST9$Group <- NA
ST9$Source <- NA
ST9[(ST9$Accession %in% GOIs$plastid),"Group"] <- "Plastid-localized"
ST9[(ST9$Accession %in% GOIs$plastid),"Source"] <- "Friso et al. (2010)"
ST9[(ST9$Accession %in% GOIs$PhANGs),"Group"] <- "PhANGs"
ST9[(ST9$Accession %in% KEGG.zma00195.digenomic$Accession),"Source"] <- "KEGG zma00195"
ST9[(ST9$Accession %in% KEGG.zma00196$Accession),"Source"] <- "KEGG zma00196"
ST9[(ST9$Accession %in% CornCycLightRxns.digenomic$Gene.Accession),"Source"] <- "CornCyc Photosynthesis Light Reactions"
ST9[(ST9$Accession %in% NE.Rubisco),"Source"] <- "MaizeGDB"
ST9[(ST9$Accession %in% GOIs$PhAPGs),"Group"] <- "PhAPGs"
ST9[(ST9$Accession %in% GOIs$PhAPGs),"Source"] <- "NCBI"
ST9[(ST9$Accession %in% GOIs$NE.PtRibo),"Group"] <- "NE plastid ribosome"
ST9[(ST9$Accession %in% GOIs$NE.PtRibo),"Source"] <- "NCBI and maize-GAMER GO"
ST9[(ST9$Accession %in% GOIs$PE.ribo),"Group"] <- "PE plastid ribosome"
ST9[(ST9$Accession %in% GOIs$PE.ribo),"Source"] <- "NCBI"
ST9[(ST9$Accession %in% GOIs$CytoRibo),"Group"] <- "Cytosolic ribosome"
ST9[(ST9$Accession %in% GOIs$CytoRibo),"Source"] <- "NCBI"
ST9[(ST9$Accession %in% GOIs$SAMS),"Group"] <- "SAM synthase"
ST9[(ST9$Accession %in% GOIs$SAMS),"Source"] <- "CornCyc 2.5.1.6"
ST9[(ST9$Accession %in% GOIs$ACS),"Group"] <- "ACC synthase"
ST9[(ST9$Accession %in% GOIs$ACS),"Source"] <- "CornCyc 4.4.1.14"
ST9[(ST9$Accession %in% GOIs$ACO),"Group"] <- "ACC oxidase"
ST9[(ST9$Accession %in% GOIs$ACO),"Source"] <- "CornCyc 1.14.17.4"
ST9[(ST9$Accession %in% GOIs$acs2),"Group"] <- "ACS2"
ST9[(ST9$Accession %in% GOIs$acs2),"Source"] <- "MaizeGDB"
ST9[(ST9$Accession %in% GOIs$acs6),"Group"] <- "ACS6"
ST9[(ST9$Accession %in% GOIs$acs6),"Source"] <- "MaizeGDB"
ST9[(ST9$Accession %in% GOIs$AtACO1hom),"Group"] <- "AtACO1 homolog"
ST9[(ST9$Accession %in% GOIs$AtACO1hom),"Source"] <- "BLAST"
ST9[(ST9$Accession %in% GOIs$AtACO2hom),"Group"] <- "AtACO2 homolog"
ST9[(ST9$Accession %in% GOIs$AtACO2hom),"Source"] <- "BLAST"
ST9[(ST9$Accession %in% GOIs$AtSAM1AtSAM2hom),"Group"] <- "AtSAM1/2 homolog"
ST9[(ST9$Accession %in% GOIs$AtSAM1AtSAM2hom),"Source"] <- "BLAST"
ST9[(ST9$Accession %in% GOIs$CarbonFixation),"Group"] <- "Carbon fixation"
ST9[(ST9$Accession %in% GOIs$CarbonFixation),"Source"] <- "KEGG zma00710"
ST9[(ST9$Accession %in% GOIs$TPR),"Group"] <- "TPR"
ST9[(ST9$Accession %in% GOIs$TPR),"Source"] <- "NCBI"
ST9[(ST9$Accession %in% GOIs$PTAC),"Group"] <- "PTAC"
ST9[(ST9$Accession %in% GOIs$PTAC),"Source"] <- "NCBI"
ST9[(ST9$Accession %in% GOIs$Oxidoreductase),"Group"] <- "Oxidoreductase"
ST9[(ST9$Accession %in% GOIs$Oxidoreductase),"Source"] <- "GAMER GO:0016491"
ST9[(ST9$Accession %in% GOIs$BiosynSecMet),"Group"] <- "Biosynthesis of secondary metabolites"
ST9[(ST9$Accession %in% GOIs$BiosynSecMet),"Source"] <- "KEGG zma01110"
ST9[(ST9$Accession %in% GOIs$Protease),"Group"] <- "Protease"
ST9[(ST9$Accession %in% GOIs$Protease),"Source"] <- "GAMER GO:0008233"
ST9[(ST9$Accession %in% GOIs$AlphaLinAcidMet),"Group"] <- "Alpha-linoleic acid metabolism"
ST9[(ST9$Accession %in% GOIs$AlphaLinAcidMet),"Source"] <- "KEGG zma00592"
ST9[(ST9$Accession %in% GOIs$ProteinBioSyn),"Group"] <- "Protein biosynthesis"
ST9[(ST9$Accession %in% GOIs$ProteinBioSyn),"Source"] <- "NCBI"

ST9 <- ST9[order(ST9$Group),]

write.csv(ST9, paste0(PathToTables,"SupplementalTable9.csv"), row.names = F)
```
