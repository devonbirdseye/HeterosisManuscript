...
#Trim to digenomic proteins (excludes ferredoxins)
KEGG.zma00195.digenomic <- KEGG.zma00195[c(1:65,86:98),]
#Get v4 accessions
...
CornCycLightRxns <- read.csv(paste0(PathToData, "CornCycLightRxns.csv"), stringsAsFactors = F)
#Trim to digenomic proteins
...
acs6 <- "Zm00001d033862"
aco2 <- "Zm00001d020686"
sams1 <- "Zm00001d010920"
```
```{r etbio TFs}
tcptf7 <- "Zm00001d010730"
tcptf23 <- "Zm00001d038683"
```
```{r}
mgdb$v4_gene_model[mgdb$arabidopsis_ortholog=="AT1G28370"]
```
```{r}
tmt.SL.etbio[tmt.SL.etbio$Gene=="Zm00001d047166",]
```
```{r}
...
```
```{r}
walley.detected[order(-walley.detected$PearsonCor),]
```
```{r}
mgdb$v4_gene_model[mgdb$arabidopsis_ortholog=="AT5G67160"]
```
```{r}
test <- tmt.SL.etbio[tmt.SL.etbio$Gene %in% SAMMTs,]
mean(test$HYB.MP)
```

```{r}
TransZeatin <- read.delim("TransZeatinGenesCornCyc.txt", stringsAsFactors = F)
TransZeatin <- TransZeatin$Gene.Accession
```

```{r}
SAMMTs <- gamer$db_object_id[gamer$term_accession=="GO:0008757"]
```
```{r}
gamer$db_object_id[gamer$term_accession=="GO:0008930"]
```
```{r}
tmt.SL.etbio[tmt.SL.etbio$Gene %in% GA,]
```
```{r}
mean(cpm.6H.etbio$A682xMo17.MP[cpm.6H.etbio$Gene %in%ABA])
```
```{r}
tmt.acs.zea[tmt.acs.zea$Gene =="Zm00001d051416",]
```
```{r ABA}
ABA <- tibble::tribble(
       ~Gene,                                                                            ~Name,
...
```
```{r}
cpm.SL.entrez <- merge.data.frame(cpm.SL, mgdb[,c("v4_gene_model", "Entrez")], by.x = "Gene", by.y = "v4_gene_model", all=F)
test <- cpm.SL[grep(x = cpm.SL$Gene, pattern = "Zm00"),]
write.csv(test[,c("Gene", "l2.B73xMo17.MP")], "cpm.SL.csv", row.names = F)
```
...
##GOI list
```{r make GOIs list}
GOIs <- list(PE.ribo, PhANGs, PhAPGs, plastid, NE.PtRibo, CytoRibo, EtBio, acs2, aco2, sams1, SAMS, ACS, ACO)
names(GOIs) <- c("PE.ribo", "PhANGs", "PhAPGs", "plastid", "NE.PtRibo", "CytoRibo", "EtBio", "acs2",  "aco2", "sams1", "SAMS", "ACS", "ACO")
saveRDS(GOIs, "GOIs")
```
```{r read GOI list}
GOIs <- readRDS("GOIs")
...
colors.ea <- c("Other"="grey", "SAMS"="black", "ACS"="black", "ACO"="black", "ZmACS2"="#24ff24", "ZmACO2"="#ff6db6", "ZmSAMS1"="#006ddb")
...
###factor
df.lims.plastid$Category <- factor(df.lims.plastid$Category, levels = c("Other", "Plastid", "PhANGs", "NE plastid ribosome", "PE plastid ribosome", "PhAPGs"))
...
df.lims.plastid <- df.lims.plastid[(order(df.lims.plastid$Order)),]
###factor
df.lims.plastid$Category <- factor(df.lims.plastid$Category, levels = c("Other", "Plastid", "PhANGs", "NE plastid ribosome"))
    #scatter plot with marginal density curves
    df.lims.plastid.scat <- ggplot(df.lims.plastid, mapping=aes(x=l2.B73xMo17.MP, y=l2.B73.Mo17, color=Category, shape=Category))+
    geom_point(aes(color=Category), size=1)+
  scale_color_manual(values=colors.p, name="Category")+
...
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$aco2),"Category"] <- "ZmACO2"
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$aco2),"Order"] <- 6
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$sams1),"Category"] <- "ZmSAMS1"
tmt.SL.acs[(tmt.SL.acs$Gene %in% GOIs$sams1),"Order"] <- 7
###Re-order
tmt.SL.acs <- tmt.SL.acs[(order(tmt.SL.acs$Order)),]
###factor
tmt.SL.acs$Category <- factor(tmt.SL.acs$Category, levels = c("Other", "SAMS", "ACS", "ACO", "ZmACS2", "ZmACO2","ZmSAMS1"))
```
...
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$aco2),"Category"] <- "ZmACO2"
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$aco2),"Order"] <- 6
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$sams1),"Category"] <- "ZmSAMS1"
cpm.SL.acs[(cpm.SL.acs$Gene %in% GOIs$sams1),"Order"] <- 7
###Re-order
cpm.SL.acs <- cpm.SL.acs[(order(cpm.SL.acs$Order)),]
###factor
cpm.SL.acs$Category <- factor(cpm.SL.acs$Category, levels = c("Other", "SAMS", "ACS", "ACO", "ZmACS2", "ZmACO2","ZmSAMS1"))
```
...
tmt.SL.acs.vol <- ggplot(tmt.SL.acs, mapping=aes(x=l2.B73xMo17.MP, y=l10.ttest, color=Category, shape=Category))+
  ...
  scale_shape_manual(values=c(1,22,23,24,19,19,19))+
    theme_classic()+
          ggtitle("A")+
          geom_hline(yintercept=1.30103, linetype="dashed", color = "gray50")+
          geom_vline(xintercept=0, linetype="dashed", color = "gray50")+
...
design <- "AB
           CC"
plotpanel<-patchwork::wrap_plots(A=tmt.SL.acs.vol, B=cpm.SL.acs.vol, C=cpm.SL.acs.vol.legend,  nrow = 2, heights = c(3,1.2), design = design)
...
```{r}
SupTab3a <- (tmt.BxM.6H.entrez.nodups.trim.pv.df[,c("labels", "all.mapped")])
SupTab3a <- separate(SupTab3a, 'all.mapped', paste("all.mapped", 1:4, sep="."), sep=",", extra="drop")
SupTab3a <- melt(SupTab3a, id.vars="labels")
SupTab3a <- SupTab3a[,c("labels", "value")]
SupTab3a$value[SupTab3a$value==""] <- NA
SupTab3a <- na.omit(SupTab3a)
colnames(SupTab3a) <- c("labels", "Gene.entrez")

SupTab3b <- (tmt.acs.entrez.nodups.trim.pv.df[,c("labels", "all.mapped")])
SupTab3b <- separate(SupTab3b, 'all.mapped', paste("all.mapped", 1:4, sep="."), sep=",", extra="drop")
SupTab3b <- melt(SupTab3b, id.vars="labels")
SupTab3b <- SupTab3b[,c("labels", "value")]
SupTab3b$value[SupTab3b$value==""] <- NA
SupTab3b <- na.omit(SupTab3b)
colnames(SupTab3b) <- c("labels", "Gene.entrez")

SupTab3 <- merge.data.frame(SupTab3a, SupTab3b, by.x = "labels", by.y = "labels", all=T)
SupTab3 <- unique(SupTab3[,c(1,3)])
colnames(SupTab3) <- c("labels", "Gene.entrez")
SupTab3 <- merge.data.frame(SupTab3, mgdb[,c("v4_gene_model", "Entrez")],by.x="Gene.entrez", by.y="Entrez", all.x = T, all.y = F)
SupTab3 <- merge.data.frame(SupTab3, tmt.6H[,c("Gene", "l2.B73xMo17.MP")], by.x = "v4_gene_model", by.y = "Gene", all.x = T, ally.=F)
SupTab3 <- merge.data.frame(SupTab3, tmt.acs[,c("Gene", "l2.ACS.B73")], by.x = "v4_gene_model", by.y = "Gene", all.x = T, ally.=F)
write.csv(SupTab3, "SupplementalTable3.csv", row.names = F)
```
...
```{r}
SupTab1 <- Reduce(function(x,y) merge(x,y,by="Accession",all=TRUE) ,list(tmt.SL[,c("Accession", "HYB.MP")], tmt.LB[,c("Accession", "HYB.MP")], tmt.6H[,c("Accession", "B73xMo17.MP")], tmt.RIL[,c("Accession", "BM.BM.MP")]))

colnames(SupTab1) <- c("Accession", "SeedlingLeaf_BxM.MP", "LeafBlade_BxM.MP", "6Hybrids_BxM.MP", "RILhybrids_BxM.MP")

write.csv(SupTab1, "SupplementalTable1.csv", row.names = F)
```
...
tmt.RIL.6H.Hyb2MP.cor <- CorFun(tmt.RIL.6H.Hyb2MP, 2:16, c(PlantHeights.ril.MP, PlantHeights.6h.MP))
#Set GOIs
SupTab2 <- tmt.RIL.6H.Hyb2MP.cor[,c(17,1,18)]
colnames(SupTab2) <- c("Gene", "Accession", "PearsonCor")
SupTab2$Category <- "Other"
SupTab2$Category[SupTab2$Gene %in% GOIs$plastid] <- "Plastid"
SupTab2$Category[SupTab2$Gene %in% GOIs$PE.ribo] <- "PE plastid ribosome"
SupTab2$Category[SupTab2$Gene %in% GOIs$NE.PtRibo] <- "NE plastid ribosome"
SupTab2$Category[SupTab2$Gene %in% GOIs$PhANGs] <- "PhANGs"
SupTab2$Category[SupTab2$Gene %in% GOIs$PhAPGs] <- "PhAPGs"
SupTab2$Category[SupTab2$Gene %in% GOIs$CytoRibo] <- "Cytosolic ribosome"
SupTab2$Category[SupTab2$Gene %in% GOIs$EtBio] <- "Ethylene biosynthesis"
SupTab2$Category[SupTab2$Gene %in% GOIs$acs2] <- "ACS2"
SupTab2$Category[SupTab2$Gene %in% GOIs$acs6] <- "ACS6"
SupTab2$Category[SupTab2$Gene %in% GOIs$sams1] <- "SAMS1"
#Re-order
SupTab2 <- SupTab2[(order(-SupTab2$PearsonCor)),]
