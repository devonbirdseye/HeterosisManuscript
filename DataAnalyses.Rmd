...
#combine setup into one code chunk, fewer lines, cleaner, faster to run
knitr::opts_chunk$set(echo = TRUE)
#set seed for reproducibility
set.seed(586)
#load packages
#we should consider using packrat for reproducibility
#add bioconductor dependency IRanges
library(IRanges)
library(readr)
library(tibble)
library(plyr)
...
library(cowplot)
library(patchwork)
#had to swap order of pkgs to avoid conflict
...
#Path to input data
#PathToData <- "Data/"
PathToData<-"C:/Users/laura/Documents/Heterosis/2020Paper/codeReview/"
PathToPlots<-"C:/Users/laura/Documents/Heterosis/2020Paper/codeReview/plots/"
...
#Convert NAs to 0
#We need to be really careful here and should not replace na with 0. 
#There are 784930 NAs produced by the above data type conversion. 
#They are all coming from "#DIV/0!" from excel which != 0
#there are also 1275 actual 0 values that shouldn't be mixed with div/0
#tmt[is.na(tmt)] <- 0
```
...
#Remove proteins not detected in any of the samples and with any NA observations by adding which()
#if the table changes, it will be tedious to change all the column numbers. I've added greps instead for clarity
tmt.BxM.OG <- tmt.BxM.OG[which(rowSums(tmt.BxM.OG[,grep(colnames(tmt.BxM.OG), pattern="SeedlingLeaf")]) > 0),]
#Calculate averages of bio reps
tmt.BxM.OG$B73avg <- rowMeans(tmt.BxM.OG[,grep(colnames(tmt.BxM.OG), pattern="B73")])
tmt.BxM.OG$Mo17avg <- rowMeans(tmt.BxM.OG[,grep(colnames(tmt.BxM.OG), pattern="Mo17")])
tmt.BxM.OG$HYBavg <- rowMeans(tmt.BxM.OG[,grep(colnames(tmt.BxM.OG), pattern="BM")])
#in this case, taking the average of the whole set is OK because there are the same number of observations of each parent. 
#If there were not, this is not the equivalent of taking the average of the parent means. 
#This gets dangerous when we move to the RNA samples where there are not the same number of observations.
#instead, lets take the mean of the means for each genotype.
#tmt.BxM.OG$MPavg <- rowMeans(tmt.BxM.OG[,c(10:15)])
tmt.BxM.OG$MPavg <- rowMeans(tmt.BxM.OG[,c("B73avg", "Mo17avg")])
#calculate ratios
#Do we ever do anything with the non-log-transformed ratios? If not, we can save a lot of lines of code/potential mistakes by log transforming at the ratio step.
...
#mostly the same changes from the "OG" Seedling leaf chunk above
#BxM from 6-hybrid experiment (6H)
tmt.BxM.6H <- tmt[,c(171:176,181:183,31,41,51,33,43,53,35,45,55)]
#Remove proteins not detected in any of the samples or with NAs
tmt.BxM.6H <- tmt.BxM.6H[which(rowSums(tmt.BxM.6H[,grep(colnames(tmt.BxM.6H), pattern = "X6Hybrid")]) > 0),]
#Calculate averages of bio reps
tmt.BxM.6H$B73avg <- rowMeans(tmt.BxM.6H[,grep(colnames(tmt.BxM.6H), pattern = "B73\\.")])
tmt.BxM.6H$Mo17avg <- rowMeans(tmt.BxM.6H[,grep(colnames(tmt.BxM.6H), pattern = "Mo17\\.")])
tmt.BxM.6H$HYBavg <- rowMeans(tmt.BxM.6H[,grep(colnames(tmt.BxM.6H), pattern = "B73xMo17")])
tmt.BxM.6H$MPavg <- rowMeans(tmt.BxM.6H[,c("B73avg", "Mo17avg")])
#calculate ratios
...
#same modifications made as for seedling leaf OG chunk
#BxM from RIL experiment (RIL)
tmt.BxM.RIL <- tmt[,c(171:176,181:183,81:90)]
#Remove proteins not detected in any of the samples
tmt.BxM.RIL <- tmt.BxM.RIL[which(rowSums(tmt.BxM.RIL[,grep(colnames(tmt.BxM.RIL), pattern="B73|Mo17|BM")]) > 0),]
#Calculate averages of bio reps
tmt.BxM.RIL$B73avg <- rowMeans(tmt.BxM.RIL[,grep(colnames(tmt.BxM.RIL), pattern="B73")])
tmt.BxM.RIL$Mo17avg <- rowMeans(tmt.BxM.RIL[,grep(colnames(tmt.BxM.RIL), pattern="Mo17")])
tmt.BxM.RIL$HYBavg <- rowMeans(tmt.BxM.RIL[,grep(colnames(tmt.BxM.RIL), pattern="BM")])
tmt.BxM.RIL$MPavg <- rowMeans(tmt.BxM.RIL[,c("B73avg", "Mo17avg")])
#calculate ratios
...
#Remove proteins not detected in any of the samples or with NAs
tmt.BxM.DN <- tmt.BxM.DN[which(rowSums(tmt.BxM.DN[,grep(colnames(tmt.BxM.DN), pattern = "Day")]) > 0),]
#Calculate averages of bio reps
tmt.BxM.DN$B73avg <- rowMeans(tmt.BxM.DN[,grep(colnames(tmt.BxM.DN), pattern = "B73")])
tmt.BxM.DN$Mo17avg <- rowMeans(tmt.BxM.DN[,grep(colnames(tmt.BxM.DN), pattern = "Mo17")])
tmt.BxM.DN$HYBavg <- rowMeans(tmt.BxM.DN[,grep(colnames(tmt.BxM.DN), pattern = "BM")])
tmt.BxM.DN$MPavg <- rowMeans(tmt.BxM.DN[,c("B73avg", "Mo17avg")])
#calculate ratios
...
#nearly the same modifications as seedling leaf OG
#BxM from Day vs. night experiment (DN.both)
tmt.BxM.DN.both <- tmt[,c(171:176,181:183,62:70,72:80)]
#Remove proteins not detected in any of the samples
tmt.BxM.DN.both <- tmt.BxM.DN.both[which(rowSums(tmt.BxM.DN.both[,grep(colnames(tmt.BxM.DN.both), pattern="Day|Night")]) > 0),]
#Calculate averages of bio reps
tmt.BxM.DN.both$day.B73avg <- rowMeans(tmt.BxM.DN.both[,grep(colnames(tmt.BxM.DN.both), pattern="Day\\.B73")])
tmt.BxM.DN.both$day.Mo17avg <- rowMeans(tmt.BxM.DN.both[,grep(colnames(tmt.BxM.DN.both), pattern="Day\\.Mo17")])
tmt.BxM.DN.both$day.HYBavg <- rowMeans(tmt.BxM.DN.both[,grep(colnames(tmt.BxM.DN.both), pattern="Day\\.BM")])
tmt.BxM.DN.both$day.MPavg <- rowMeans(tmt.BxM.DN.both[,c("B73avg", "Mo17avg")])

tmt.BxM.DN.both$night.B73avg <- rowMeans(tmt.BxM.DN.both[,grep(colnames(tmt.BxM.DN.both), pattern="Night\\.B73")])
tmt.BxM.DN.both$night.Mo17avg <- rowMeans(tmt.BxM.DN.both[,grep(colnames(tmt.BxM.DN.both), pattern="Night\\.Mo17")])
tmt.BxM.DN.both$night.HYBavg <- rowMeans(tmt.BxM.DN.both[,grep(colnames(tmt.BxM.DN.both), pattern="Night\\.BM")])
tmt.BxM.DN.both$night.MPavg <- rowMeans(tmt.BxM.DN.both[,c("B73avg", "Mo17avg")])
#calculate ratios
...
#Remove proteins not detected in any of the samples
tmt.6H.all6 <- tmt.6H.all6[which(rowSums(tmt.6H.all6[,grep(colnames(tmt.6H.all6),pattern = "6Hybrid")]) > 0),]
#calculate averages
tmt.6H.all6$B73 <- rowMeans(tmt.6H.all6[,grep(colnames(tmt.6H.all6),pattern = "B73\\.")])
tmt.6H.all6$Mo17 <- rowMeans(tmt.6H.all6[,grep(colnames(tmt.6H.all6),pattern = "6Hybrid.Mo17")])
tmt.6H.all6$B73xMo17 <- rowMeans(tmt.6H.all6[,grep(colnames(tmt.6H.all6),pattern = "B73xMo17")])
tmt.6H.all6$Mo17xB73 <- rowMeans(tmt.6H.all6[,grep(colnames(tmt.6H.all6),pattern = "Mo17xB73")])
tmt.6H.all6$B84 <- rowMeans(tmt.6H.all6[,grep(colnames(tmt.6H.all6),pattern = "6Hybrid.B84\\.")])
tmt.6H.all6$B84xB73 <- rowMeans(tmt.6H.all6[,grep(colnames(tmt.6H.all6),pattern = "B84xB73")])
tmt.6H.all6$B84xMo17 <- rowMeans(tmt.6H.all6[,grep(colnames(tmt.6H.all6),pattern = "B84xMo17")])
tmt.6H.all6$A682 <- rowMeans(tmt.6H.all6[,grep(colnames(tmt.6H.all6),pattern = "6Hybrid.A682\\.")])
tmt.6H.all6$A682xB73 <- rowMeans(tmt.6H.all6[,grep(colnames(tmt.6H.all6),pattern = "A682xB73")])
tmt.6H.all6$A682xMo17 <- rowMeans(tmt.6H.all6[,grep(colnames(tmt.6H.all6),pattern = "A682xMo17")])
#Calculate hyb/MP ratios
tmt.6H.all6$B73xMo17.MP <- tmt.6H.all6$B73xMo17/rowMeans(tmt.6H.all6[,c("B73", "Mo17")])
tmt.6H.all6$Mo17xB73.MP <- tmt.6H.all6$Mo17xB73/rowMeans(tmt.6H.all6[,c("Mo17", "B73")])
tmt.6H.all6$B84xB73.MP <- tmt.6H.all6$B84xB73/rowMeans(tmt.6H.all6[,c("B84", "B73")])
tmt.6H.all6$B84xMo17.MP <- tmt.6H.all6$B84xMo17/rowMeans(tmt.6H.all6[,c("B84", "Mo17")])
tmt.6H.all6$A682xB73.MP <- tmt.6H.all6$A682xB73/rowMeans(tmt.6H.all6[,c("A682", "B73")])
tmt.6H.all6$A682xMo17.MP <- tmt.6H.all6$A682xMo17/rowMeans(tmt.6H.all6[,c("A682", "Mo17")])
...
#Remove proteins not detected in any of the samples
tmt.RIL.all9 <- tmt.RIL.all9[which(rowSums(tmt.RIL.all9[,10:99]) > 0),]
#checking the column numbers for the rest of this chunk is a little tedious
#instead, assign new column names to the ranges so we can access by name; only need to check numbers once per run
colnames(tmt.RIL.all9)[10:19]<-paste0("BxM_",colnames(tmt.RIL.all9)[10:19])
colnames(tmt.RIL.all9)[20:39]<-paste0("x14_",colnames(tmt.RIL.all9)[20:39])
colnames(tmt.RIL.all9)[40:59]<-paste0("x16_",colnames(tmt.RIL.all9)[40:59])
colnames(tmt.RIL.all9)[60:79]<-paste0("x21_",colnames(tmt.RIL.all9)[60:79])
colnames(tmt.RIL.all9)[80:99]<-paste0("x317_",colnames(tmt.RIL.all9)[80:99])
#Calculate averages
#got rid of na.rm=T as per our conversation on Friday 9/11
tmt.RIL.all9$BM.B73 <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="BxM_B73")])
tmt.RIL.all9$BM.Mo17 <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="BxM_Mo17")])
tmt.RIL.all9$BM.BM <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="BxM_BM")])
tmt.RIL.all9$X14.B73 <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x14_B73")])
tmt.RIL.all9$X14.Mo17<- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x14_Mo17")])
tmt.RIL.all9$X14.14 <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x14_X14")])
tmt.RIL.all9$X14.14B <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x14_X14B")])
tmt.RIL.all9$X14.14M <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x14_X14M")])
tmt.RIL.all9$X16.B73 <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x16_B73")])
tmt.RIL.all9$X16.Mo17<- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x16_Mo17")])
tmt.RIL.all9$X16.16 <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x16_X16")])
tmt.RIL.all9$X16.16B <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x16_X16B")])
tmt.RIL.all9$X16.16M <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x16_X16M")])
tmt.RIL.all9$X21.B73 <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x21_B73")])
tmt.RIL.all9$X21.Mo17<- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x21_Mo17")])
tmt.RIL.all9$X21.21 <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x21_X21")])
tmt.RIL.all9$X21.21B <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x21_X21B")])
tmt.RIL.all9$X21.21M <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x21_X21M")])
tmt.RIL.all9$X317.B73 <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x317_B73")])
tmt.RIL.all9$X317.Mo17<- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x317_Mo17")])
tmt.RIL.all9$X317.317 <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x317_X317")])
tmt.RIL.all9$X317.317B <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x317_X317B")])
tmt.RIL.all9$X317.317M <- rowMeans(tmt.RIL.all9[,grep(colnames(tmt.RIL.all9), pattern="x317_X317B")])
#Calculate hyb/mp ratios
tmt.RIL.all9$BM.BM.MP <- tmt.RIL.all9$BM.BM/rowMeans(tmt.RIL.all9[,c("BM.B73", "BM.Mo17")])
tmt.RIL.all9$X14.14B.MP <- tmt.RIL.all9$X14.14B/rowMeans(tmt.RIL.all9[,c("X14.B73", "X14.X14")])
tmt.RIL.all9$X14.14M.MP <- tmt.RIL.all9$X14.14M/rowMeans(tmt.RIL.all9[,c("X14.Mo17","X14.X14")])
tmt.RIL.all9$X16.16B.MP <- tmt.RIL.all9$X16.16B/rowMeans(tmt.RIL.all9[,c("X16.B73","X16.X16")])
tmt.RIL.all9$X16.16M.MP <- tmt.RIL.all9$X16.16M/rowMeans(tmt.RIL.all9[,c("X16.Mo17","X16.X16")])
tmt.RIL.all9$X21.21B.MP <- tmt.RIL.all9$X21.21B/rowMeans(tmt.RIL.all9[,c("X21.B73","X21.X21")])
tmt.RIL.all9$X21.21M.MP <- tmt.RIL.all9$X21.21M/rowMeans(tmt.RIL.all9[,c("X21.Mo17","X21.X21")])
tmt.RIL.all9$X317.317B.MP <- tmt.RIL.all9$X317.317B/rowMeans(tmt.RIL.all9[,c("X317.B73","X317.X317")])
tmt.RIL.all9$X317.317M.MP <- tmt.RIL.all9$X317.317M/rowMeans(tmt.RIL.all9[,c("X317.Mo17","X317.X317")])
#Calculate parent/parent ratios
...
#Remove proteins not detected in any of the samples
tmt.acs <- tmt.acs[which(rowSums(tmt.acs[,grep(colnames(tmt.acs), pattern = "B73\\.acs")]) > 0),]
#Calculate averages of bio reps - acs
tmt.acs$B73avg <- rowMeans(tmt.acs[,grep(colnames(tmt.acs), pattern = "B73\\.r")])
tmt.acs$ACSavg <- rowMeans(tmt.acs[,grep(colnames(tmt.acs), pattern = "acs\\.r")])
#calculate ratio
...
```{r import RNA data}
#Original experiment
cpm.OG <- as.data.frame(read_tsv(paste0(PathToData, "cpm.tsv")))
#read in sample metadata from original RNAseq
cpm.OG.Samp<-as.data.frame(read_tsv(paste0(PathToData, "samples.tsv")))
#6-hybrid experiment
cpm.6H <- readRDS(paste0(PathToData, "rn20b.rds"))
cpm.6H.tm <- cpm.6H$tm
cpm.6H.tm_m <- cpm.6H$tm_m
#get sample names
cpm.6H.Samp <- as.data.frame(cpm.6H$th)
#get rid of large object to save memory
rm(cpm.6H)
#RIL experiment
...
#get sample names
cpm.RIL.Samp<-as.data.frame(cpm.RIL$th)
#get rid of large object to save memory
rm(cpm.RIL)
```
...
#pulling the RNAseq data from the columns this way is risky. There are a couple sample swaps in the original spreadsheet
#instead get sample names from sample spreadsheet and pull columns that way
OGSamps<-colnames(cpm.OG)[-1]
OGSampsNames<-paste(cpm.OG.Samp[match(OGSamps, cpm.OG.Samp$SampleID), "Tissue"],
                    cpm.OG.Samp[match(OGSamps, cpm.OG.Samp$SampleID), "Genotype"],
                    sep = "_")
colnames(cpm.OG)<-c("Gene", OGSampsNames)
cpm.BxM.OG <- cpm.OG[,c(1,grep(colnames(cpm.OG), pattern = "seedlingleaf"))]
...
cpm.BxM.OG <- cpm.BxM.OG[!(rowSums(cpm.BxM.OG[,grep(colnames(cpm.BxM.OG), pattern="seedlingleaf")])==0),]
#Limit to genes we detect as proteins in paired experiment
cpm.BxM.OG <- cpm.BxM.OG[cpm.BxM.OG$Gene%in%tmt.BxM.OG$Gene,]
#Calculate averages of bio reps
cpm.BxM.OG$B73avg <- rowMeans(cpm.BxM.OG[,grep(colnames(cpm.BxM.OG), pattern = "B73$|B73[^x]")])
cpm.BxM.OG$Mo17avg <- rowMeans(cpm.BxM.OG[,grep(colnames(cpm.BxM.OG), pattern = "_Mo17")])
cpm.BxM.OG$HYBavg <- rowMeans(cpm.BxM.OG[,grep(colnames(cpm.BxM.OG), pattern = "B73xMo17")])
cpm.BxM.OG$MPavg <- rowMeans(cpm.BxM.OG[,c("B73avg", "Mo17avg")])
#calculate ratios
...
```{r subset BxM from 6-hybrid experiment}
#this next segment is a little more complicated than what you wrote, but it's safer in case the gids aren't identical in each sample
#get B73/Mo17/BM samples
B73Samp<-cpm.6H.Samp$SampleID[which(cpm.6H.Samp$Tissue=="leaf_V2"&cpm.6H.Samp$Genotype=="B73"&cpm.6H.Samp$Treatment=="m")]
Mo17Samp<-cpm.6H.Samp$SampleID[which(cpm.6H.Samp$Tissue=="leaf_V2"&cpm.6H.Samp$Genotype=="Mo17"&cpm.6H.Samp$Treatment=="m")]
BMSamp<-cpm.6H.Samp$SampleID[which(cpm.6H.Samp$Tissue=="leaf_V2"&cpm.6H.Samp$Genotype=="B73xMo17"&cpm.6H.Samp$Treatment=="m")]
allSamp<-c(B73Samp, Mo17Samp, BMSamp)
#subset data, get into list
cpm.BxM.6H<-sapply(allSamp, function(x) as.data.frame(cpm.6H.tm[cpm.6H.tm$SampleID==x,c("gid", "CPM")]), simplify = FALSE)
cpm.BxM.6H<-sapply(seq(1:length(cpm.BxM.6H)), function(x) {
  retDF<-cpm.BxM.6H[[x]]
  colnames(retDF)[colnames(retDF)=="CPM"]<-names(cpm.BxM.6H)[x]
  return(retDF)
  }, simplify = FALSE)
#use Reduce() to merge across lists by gid, doing full join
#Reduce is pretty handy for this kind of thing, there is also a "tidyverse" way to do it with Purr
cpm.BxM.6H<-Reduce(function(A, B) merge(A, B, by = "gid", all=T), cpm.BxM.6H)
#rename gid to Gene to match protein data
colnames(cpm.BxM.6H)[colnames(cpm.BxM.6H) == "gid"]<-"Gene"
#Remove genes not detected
cpm.BxM.6H <- cpm.BxM.6H[!(rowSums(cpm.BxM.6H[,!(colnames(cpm.BxM.6H)%in%c("Gene"))])==0),]
#limit to genes detected as proteins
cpm.BxM.6H <- cpm.BxM.6H[cpm.BxM.6H$Gene%in%tmt.BxM.6H$Gene,]
#Calculate averages of bio reps
cpm.BxM.6H$B73avg <- rowMeans(cpm.BxM.6H[,B73Samp])
cpm.BxM.6H$Mo17avg <- rowMeans(cpm.BxM.6H[,Mo17Samp])
cpm.BxM.6H$HYBavg <- rowMeans(cpm.BxM.6H[,BMSamp])
cpm.BxM.6H$MPavg <- rowMeans(cpm.BxM.6H[,c("B73avg", "Mo17avg")])
#calculate ratios
...
```{r subset BxM from RIL experiment}
#get samples
B73Samp<-cpm.RIL.Samp[which(cpm.RIL.Samp$Genotype=="B73"),"SampleID"]
Mo17Samp<-cpm.RIL.Samp[which(cpm.RIL.Samp$Genotype=="Mo17"),"SampleID"]
BMSamp<-cpm.RIL.Samp[which(cpm.RIL.Samp$Genotype=="BxM"),"SampleID"]
allSamp<-c(B73Samp, Mo17Samp, BMSamp)
#subset data, get into list
cpm.BxM.RIL<-sapply(allSamp, function(x) as.data.frame(cpm.RIL.tm[cpm.RIL.tm$SampleID==x,c("gid", "CPM")]), simplify = FALSE)
cpm.BxM.RIL<-sapply(seq(1:length(cpm.BxM.RIL)), function(x) {
  retDF<-cpm.BxM.RIL[[x]]
  colnames(retDF)[colnames(retDF)=="CPM"]<-names(cpm.BxM.RIL)[x]
  return(retDF)
  }, simplify = FALSE)
#use Reduce() to merge across lists by gid, doing full join
cpm.BxM.RIL<-Reduce(function(A, B) merge(A, B, by = "gid", all=T), cpm.BxM.RIL)
#rename gid to Gene to match protein data
colnames(cpm.BxM.RIL)[colnames(cpm.BxM.RIL) == "gid"]<-"Gene"
#Remove genes not detected
cpm.BxM.RIL <- cpm.BxM.RIL[!(rowSums(cpm.BxM.RIL[,!(colnames(cpm.BxM.RIL)%in%c("Gene"))])==0),]
#limit to genes detected as protein
cpm.BxM.RIL <- cpm.BxM.RIL[cpm.BxM.RIL$Gene%in%tmt.BxM.RIL$Gene,]
#Calculate averages of bio reps
cpm.BxM.RIL$B73avg <- rowMeans(cpm.BxM.RIL[,B73Samp])
cpm.BxM.RIL$Mo17avg <- rowMeans(cpm.BxM.RIL[,Mo17Samp])
cpm.BxM.RIL$HYBavg <- rowMeans(cpm.BxM.RIL[,BMSamp])
cpm.BxM.RIL$MPavg <- rowMeans(cpm.BxM.RIL[,c("B73avg","Mo17avg")])
#calculate ratios
...
mgdb <- mgdb[-c(1:5),]
```
Any particular reason why we can't read in a table for this?
```{r organelleEntrez}
...
ribosom.pt <- ribosom.pt[grep("chloropl|plastid", ribosom.pt$GOterm),]
...
#combine RIL and 6 hybrids
cpm.RIL.6H.Hyb2MP <- merge.data.frame(cpm.RIL.all9[,c(1,17:25)], cpm.6H.all6[,c(1,12:17)], by.x = "Gene", by.y = "Gene", all = T)
...
cpm.PlastidRiboCor.hyb2MP <- cpm.RIL.6H.Hyb2MP[cpm.RIL.6H.Hyb2MP$Gene %in% c(GOIs$PE.ribo, GOIs$NE.PtRibo),]
#re-name columns
